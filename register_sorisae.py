...
try:
    ...
    # categories 테이블에서 category_id 확인
    c.execute("SELECT id FROM categories WHERE name = '통역·번역' LIMIT 1")
    cat_row = c.fetchone()
    ...
    print(f"   다운로드 URL: http://127.0.0.1:8000/api/marketplace/zip/sorisae-interpreter-v1.zip")
