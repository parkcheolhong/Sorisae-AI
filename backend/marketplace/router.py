...
def _backfill_passkey_credentials_from_users() -> dict[str, int]:
    stats = {
        "scanned": 0,
        "inserted": 0,
        "updated": 0,
    ...
    try:
        users_with_passkey = (
            ...
            .filter(models.User.passkey_enabled.is_(True))
            .all()
        )
        for user in users_with_passkey:
            stats["scanned"] += 1
        ...
        db.commit()
        logger.info(
            "[PASSKEY_BACKFILL] scanned=%s inserted=%s updated=%s unchanged=%s skipped=%s",
            stats["scanned"],
            stats["inserted"],
            stats["updated"],
            stats["unchanged"],
...
def ensure_marketplace_runtime_schema() -> None:
    ...
    try:
        _backfill_passkey_credentials_from_users()
    except Exception as exc:
        logger.warning("[WARN] passkey credential backfill failed: %s", exc)
    db = SessionLocal()
    try:
