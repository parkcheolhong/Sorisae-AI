...
def main():
    """메인 실행 함수"""
    print("🎼 소리새 감정 기반 음악 생성기")
    print("================================")

    generator = EmotionBasedMusicGenerator()

    test_emotions = [
        ("I feel really happy today!", 0.8),
        ("I'm feeling a bit sad", 0.6),
        ("I need energetic music", 0.9),
        ("Something calm please", 0.5)
    ]

    compositions = []
    for emotion_text, intensity in test_emotions:
        composition = generator.create_musical_composition(emotion_text, intensity)
        compositions.append(composition)

    with open('generated_compositions.json', 'w', encoding='utf-8') as f:
        json.dump(compositions, f, ensure_ascii=False, indent=2)

    print(f"\n💾 {len(compositions)}개의 음악 작품이 저장되었습니다.")
