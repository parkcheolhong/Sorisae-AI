...
class HybridConversationSystem:
        ...
        if VOICE_AVAILABLE:
            ...
            if self.audio_io_enabled:
                try:
                    ...
                    print(f"⚠️ 기본 입력 장치를 찾을 수 없습니다. 헤드리스 대화 모드로 전환합니다: {e}")
                    self.audio_io_enabled = False
                try:
                    self.tts = pyttsx3.init()
                except Exception as e:
                    print(f"⚠️ 대화 TTS 엔진 초기화 실패. 텍스트 대화 모드로 유지합니다: {e}")
                    self.tts = None
            else:
                print("ℹ️ 헤드리스 대화 모드 - 음성 입출력을 비활성화합니다.")
            self.setup_voice()
    ...
    def setup_voice(self):
        """음성 설정"""
        if VOICE_AVAILABLE and self.microphone and self.tts:
            try:
                # 한국어 음성 찾기
    ...
    def process_conversation(self):
        ...
        if not self.microphone:
            self.speak("현재 환경에서는 마이크 기반 대화를 사용할 수 없습니다.")
            return
    ...
    def start_translation_mode(self):
        """통역 모드 시작"""
        if not self.microphone:
            print("⚠️ 현재 환경에서는 마이크 기반 통역 모드를 사용할 수 없습니다.")
            return
    ...
    def process_translation(self):
        """번역 처리"""
        if not self.microphone:
            self.speak("현재 환경에서는 마이크 기반 번역을 사용할 수 없습니다.")
            return
