...
from modules.sorisae_dashboard_web import broadcast_voice_command, broadcast_system_status, broadcast_persona_change, broadcast_creative_activity
from nlp_processor import NLPProcessor
from self_learning_engine import SelfLearningEngine
from auto_feature_expansion import AutoFeatureExpansion
from creative_sorisae_engine import CreativeSorisaeEngine
...
class SorisaeCore:
    def __init__(self, config_path="config/settings.json"):
        ...
        # 🎨 창조형 소리새 엔진 초기화
        self.creative_engine = CreativeSorisaeEngine()
        ...
        self.guide_context_enabled = True
    ...
    def speak_with_emotion(self, text, emotion="neutral", speed_modifier=1.0):
        ...
        try:
            ...
            if self.guide_context_enabled:
                guide_prefix = self._build_guide_context(self.current_command)
                text = f"{guide_prefix}. {text}" if text else guide_prefix
    ...
    def _resolve_gps_location(self):
        """설정 또는 환경변수에서 GPS 위치를 해석한다."""
        location_sources = [
            self.config.get("guide_location", {}),
            self.config.get("gps", {}),
            self.config,
        ]
        env_sources = [
            {
                "latitude": os.getenv("SORISAE_GPS_LATITUDE"),
                "longitude": os.getenv("SORISAE_GPS_LONGITUDE"),
                "label": os.getenv("SORISAE_GPS_LOCATION_NAME"),
            }
        ]

        for source in [*location_sources, *env_sources]:
            latitude = source.get("latitude") or source.get("lat")
            longitude = source.get("longitude") or source.get("lon")
            label = (
                source.get("label")
                or source.get("name")
                or source.get("location_name")
                or source.get("gps_name")
            ...
            return {
                ...
                "label": label or "GPS 위치",
            }

        return None

    def _format_location_text(self, location):
        if not location:
            return "GPS 위치 미설정"

        label = str(location.get("label") or "GPS 위치").strip()
        latitude = location.get("latitude")
        longitude = location.get("longitude")
        if latitude is None or longitude is None:
            return label
        return f"{label} ({latitude:.5f}, {longitude:.5f})"

    def _build_guide_context(self, command: str = "") -> str:
        now = datetime.now()
        weekday_names = ["월", "화", "수", "목", "금", "토", "일"]
        weekday = weekday_names[now.weekday()]
        location_text = self._format_location_text(self.current_location)

        memory_hint = ""
        if command and hasattr(self, "memory_palace"):
            try:
                memories = self.memory_palace.recall_memories(command, 2)
            except Exception:
                memories = []
            if memories:
                recent = []
                for memory in memories[:2]:
                    user_input = str(memory.get("user_input") or "").strip()
                    timestamp = str(memory.get("timestamp") or "").strip()
                    if user_input:
                        summary = user_input[:18]
                        if timestamp:
                            summary += f" ({timestamp[:10]})"
                        recent.append(summary)
                if recent:
                    memory_hint = f" | 기억: {'; '.join(recent)}"

        return (
            f"[가이드] {now.year}년 {now.month}월 {now.day}일 {weekday}요일 "
            f"{now.strftime('%H:%M')} | 위치: {location_text}{memory_hint}"
        )

    def _attach_guide_context(self, command: str, response: str) -> str:
        response_text = str(response or "").strip()
        if not response_text:
            return self._build_guide_context(command)

        guide_context = self._build_guide_context(command)
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.current_location = self._resolve_gps_location()
            ...
            if smart_response:
                ...
                self.memory_palace.remember_conversation(
                    cmd,
                    self._attach_guide_context(cmd, final_response),
                    emotion,
                )
                yield cmd
