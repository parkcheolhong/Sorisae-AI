...
def main(context: dict = None) -> dict:
    """dispatch API용 메인 - 애니메이션 스튜디오 시나리오 분석"""
    context = context or {}
    scenario_text = str(context.get('scenario', '장면 1: 주인공이 도시를 걷는다.\n장면 2: 갑자기 비가 내리기 시작한다.\n장면 3: 피날레 - 두 사람이 재회한다.'))
    movie_title = str(context.get('title', '소리새 애니메이션'))

    try:
        analyzer = ScenarioAnalyzer()
        scenes = analyzer.analyze_scenario(scenario_text)
