...
# 기존 시스템 import
try:
    from modules.ai_code_manager.sorisae_core_controller import SorisaeCore # type: ignore
...
# 하이브리드 시스템 import
try:
    from hybrid_voice_processor import HybridVoiceProcessor # type: ignore
    from sorisae_integrated_hybrid_system import SorisaeIntegratedHybridSystem # pyright: ignore[reportMissingImports]
...
try:
    from sorisae_dual_brain_stock_system import StockDualBrainSystem # type: ignore
    from stock_prediction_200_percent import StockMarket200PercentPredictor # type: ignore
...
class SorisaeIntelligentInvestmentAdvisor:
    ...
    def __init__(self):
        ...
        if HYBRID_AVAILABLE:
            try:
                self.hybrid_system = SorisaeIntegratedHybridSystem() # type: ignore
                self.voice_processor = HybridVoiceProcessor() # type: ignore
        ...
        if STOCK_SYSTEMS_AVAILABLE:
            try:
                self.stock_predictor = StockMarket200PercentPredictor() # type: ignore
                self.dual_brain_system = StockDualBrainSystem() # type: ignore
        ...
        if SORISAE_CORE_AVAILABLE:
            try:
                self.sorisae_core = SorisaeCore() # type: ignore
...
class SorisaeInvestmentAdvisor(SorisaeIntelligentInvestmentAdvisor):
    ...
    def initialize_investment_system(self):
        ...
        try:
            ...
            self.dual_brain_system.start_dual_brain_system() # type: ignore
...
🚀 지속 발전하는 적중률로 투자 성공을 보장합니다!
    ...
    def analyze_stock_voice(self, command: str) -> str:
        ...
        # 200% 분석 실행
        prediction = self.stock_predictor.analyze_stock_200_percent(detected_stock) # type: ignore
...
🌟 200% 정확도로 분석되었습니다!
    ...
    def dual_brain_analyze_stock_voice(self, command: str) -> str:
        ...
        # 🧠🧠 듀얼브레인 직접 예측
        dual_brain_pred = self.dual_brain_system.predict_stock_with_dual_brain(detected_stock) # type: ignore

        # 200% 강화 분석도 실행
        enhanced_prediction = self.stock_predictor.analyze_stock_200_percent(detected_stock) # type: ignore

        # 진화 통계 획득
        evolution_stats = self.dual_brain_system.get_evolution_statistics() # type: ignore
...
🚀 듀얼브레인이 계속 진화하며 적중률이 상승합니다!
    ...
    def market_outlook_voice(self) -> str:
        """시장 전망 음성 응답"""
        market_pred = self.stock_predictor.predict_market_index("KOSPI") # type: ignore
...
✨ 확실도: {market_pred['certainty_level']}
    ...
    def trading_signals_voice(self) -> str:
        """매매 신호 음성 응답"""
        signals = self.stock_predictor.generate_trading_signals(["AAPL", "MSFT", "TSLA"]) # type: ignore
    ...
    def run_investment_advisor(self):
        ...
        try:
            ...
            if hasattr(self, 'dual_brain_system'):
                self.dual_brain_system.dual_brain_active = False # type: ignore
                print("💤 듀얼브레인 시스템 종료")

        print("\n🧠🧠 소리새 듀얼브레인 투자 어드바이저를 이용해 주셔서 감사합니다!")


def main(context: dict = None) -> dict: # type: ignore
    """🧠🧠 듀얼브레인 메인 실행 함수 (dispatch 호환)"""
    context = context or {}
    advisor = SorisaeIntelligentInvestmentAdvisor()
    return {
        'status': 'ok',
        'system': 'investment_advisor_200',
        'dual_brain_active': True,
        'query': str(context.get('query', '투자 전략 분석')),
        'risk_profile': str(context.get('risk_profile', 'balanced')),
    }


def _main_full():
    """🧠🧠 소리새 듀얼브레인 투자 어드바이저 200%+ 시스템 시작!"""
