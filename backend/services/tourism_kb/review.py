...
from backend.api_public_messages import tourism_stats_unavailable_payload
...
class TourismReviewStore:
    ...
    def stats(self) -> Dict[str, Any]:
        """Return review-label aggregates.

        ``available`` reflects whether metrics are safe to render:
        - ``False`` when the store is disconnected or aggregation failed.
        - ``True`` when label counts and precision fields are populated.
        On failure, ``error`` is a stable code (``stats_unavailable``), not exception text.
        """
        if not self._engine:
        ...
        try:
            ...
            return tourism_stats_unavailable_payload()
