...
_friend_diag_lock = threading.Lock()
_friend_diag_state: dict[str, Any] = {
    "fallback_count": 0,
...
def _friend_diag_snapshot() -> dict[str, Any]:
    with _friend_diag_lock:
        return {
            "fallback_count": int(_friend_diag_state.get("fallback_count") or 0),
            "last_fallback_reason": str(_friend_diag_state.get("last_fallback_reason") or ""),
            "last_fallback_error": str(_friend_diag_state.get("last_fallback_error") or ""),
            "last_base_url": str(_friend_diag_state.get("last_base_url") or ""),
            "last_model": str(_friend_diag_state.get("last_model") or ""),
            "last_served_models": list(_friend_diag_state.get("last_served_models") or []),
        }


def _friend_diag_update_runtime(base_url: str, model: str, served_models: set[str]) -> None:
    with _friend_diag_lock:
        _friend_diag_state["last_base_url"] = base_url
        _friend_diag_state["last_model"] = model
        _friend_diag_state["last_served_models"] = sorted(served_models)


def _friend_diag_mark_fallback(reason: str, error_text: str = "") -> None:
    with _friend_diag_lock:
        _friend_diag_state["fallback_count"] = int(_friend_diag_state.get("fallback_count") or 0) + 1
        _friend_diag_state["last_fallback_reason"] = reason
        _friend_diag_state["last_fallback_error"] = error_text
...
def _resolve_friend_chat_model(base_url: Optional[str] = None, served_models: Optional[set[str]] = None) -> str:
    """친구 모드 모델 해석.
    ...
    base = base_url or _friend_chat_base_url()
    served = served_models if served_models is not None else _list_served_models(base)
...
async def _friend_chat_completion(
    ...
    base_url = _friend_chat_base_url()
    served_models = _list_served_models(base_url)
    resolved_model = _resolve_friend_chat_model(base_url=base_url, served_models=served_models)
    _friend_diag_update_runtime(base_url, resolved_model, served_models)
    logger.info(
        "[voice/friend-chat] runtime base=%s model=%s served=%d timeout=%.1fs max_tokens=%d",
        base_url,
        resolved_model,
        len(served_models),
        VOICE_FRIEND_TIMEOUT_SEC,
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"친구 모드 LLM 응답 오류(status={response.status_code}, base={base_url}, model={resolved_model})"
        )
    data = response.json()
...
def _friend_fallback_reply(language: Optional[str]) -> str:
    """친구 모드 업스트림 장애 시 반환하는 경량 안전 폴백 문장."""
    lang = str(language or "").strip().lower()
    if lang.startswith("ko"):
        return "지금 답변 엔진 연결이 잠시 불안정해요. 잠깐 뒤에 다시 말해주면 바로 이어서 도와줄게요."
    if lang.startswith("ja"):
        return "今は応答エンジン接続が一時的に不安定です。少ししてからもう一度話しかけてください。"
    return "The reply engine is temporarily unstable right now. Please try speaking again in a moment."
...
def _friend_is_noise_or_hallucination(transcript: str) -> bool:
    ...
    # 무음에서 빈번히 뜨는 단일 환각 토큰("You", "Thanks")은 즉시 차단한다.
    normalized_ascii = re.sub(r"[^a-z0-9 ]+", "", text.lower()).strip()
...
async def voice_friend_chat(request: FriendChatRequest):
        ...
        try:
            ...
            if not transcript:
                try:
                    ...
                    language_probability = float(whisper_payload.get("language_probability", 0.0) or 0.0)
                    stt_trust = str(whisper_payload.get("stt_trust") or "high")
                except Exception as exc:
                    stt_errors.append(f"faster-whisper: {exc}")

            if not transcript:
                stt_error_message = f"STT 실패: {' | '.join(stt_errors) or 'no speech detected'}"
                status_code = _voice_stt_exc_http_status(stt_error_message)
                if status_code == 422:
                    raise HTTPException(status_code=422, detail="음성이 감지되지 않았습니다. 다시 말씀해 주세요.")
                raise HTTPException(status_code=400, detail=stt_error_message)
        except HTTPException:
            raise
        except Exception as exc:
            status_code = _voice_stt_exc_http_status(str(exc))
            if status_code == 422:
                raise HTTPException(status_code=422, detail="음성이 감지되지 않았습니다. 다시 말씀해 주세요.")
            raise HTTPException(status_code=400, detail=f"STT 실패: {exc}")
    ...
    # 무음/잡음 자기발화 방지 — Whisper 신뢰도 낮음이면 원칙적으로 차단하고,
    # 한국어 발화로 판정되는 경우에만 제한적으로 허용한다.
    if not _friend_accept_despite_low_stt_trust(
        transcript,
        detected_language,
        language_probability,
        stt_trust,
    ) and (
        str(stt_trust).strip().lower() == "low"
        or _friend_is_noise_or_hallucination(transcript)
    ):
        logger.info(
            "[voice/friend-chat] rejected low-trust/noise input trust=%s lang=%s prob=%.3f text=%s",
            stt_trust,
            detected_language,
            language_probability,
            _anonymize_text_for_log(transcript),
        )
        raise HTTPException(status_code=422, detail="음성이 감지되지 않았습니다. 다시 말씀해 주세요.")
    ...
    try:
        ...
        _friend_diag_mark_fallback("llm_exception", str(exc))
        response_text = _friend_fallback_reply(reply_language)

    # 음성/표시용 정리 — 마크다운(**, #, -)·*** 마스킹 제거(단말 TTS '별표' 발화 방지).
    response_text = _sanitize_friend_reply_for_speech(response_text)

    if not response_text:
        _friend_diag_mark_fallback("empty_response", "empty response after sanitize")
        response_text = _friend_fallback_reply(reply_language)
...
@router.get("/voice/friend-chat/diagnostics")
async def voice_friend_chat_diagnostics(_current_user: dict = Depends(get_current_user)):
    """친구 모드 런타임 진단: base URL/served models/timeout/fallback count 확인용."""
    base_url = _friend_chat_base_url()
    served_models = _list_served_models(base_url)
    model = _resolve_friend_chat_model(base_url=base_url, served_models=served_models)
    _friend_diag_update_runtime(base_url, model, served_models)
    state = _friend_diag_snapshot()
    return {
        "base_url": base_url,
        "resolved_model": model,
        "served_models": sorted(served_models),
        "timeout_sec": VOICE_FRIEND_TIMEOUT_SEC,
        "max_tokens": VOICE_FRIEND_MAX_TOKENS,
        "temperature": VOICE_FRIEND_TEMPERATURE,
        "history_turns": VOICE_FRIEND_HISTORY_TURNS,
        "web_search_enabled": VOICE_FRIEND_WEB_SEARCH,
        "fallback_count": state["fallback_count"],
        "last_fallback_reason": state["last_fallback_reason"],
        "last_fallback_error": state["last_fallback_error"],
    }
