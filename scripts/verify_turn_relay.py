...
def _build_allocate(txid: bytes, *, username: bytes = b"", realm: bytes = b"",
    ...
    if username:
        ...
        password = base64.b64encode(
            hmac.new(secret.encode(), username, hashlib.sha1).digest()  # codeql[py/weak-sensitive-data-hashing]
        )
        # codeql[py/weak-sensitive-data-hashing]
        key = hashlib.md5(username + b":" + realm + b":" + password).digest()  # codeql[py/weak-sensitive-data-hashing]
        header_len = len(attrs) + 24  # + MESSAGE-INTEGRITY attr(4+20)
        msg = struct.pack("!HHI", 0x0003, header_len, _STUN_MAGIC) + txid + attrs
        # codeql[py/weak-sensitive-data-hashing]
        integrity = hmac.new(key, msg, hashlib.sha1).digest()  # codeql[py/weak-sensitive-data-hashing]
