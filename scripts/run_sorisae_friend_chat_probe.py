...
class SorisaeProbe:
    ...
    def check_friend_chat_text(self) -> None:
        ...
        ok = status == 200 and len(text) >= MIN_RESPONSE_LEN
        self._record(
            "friend_chat_text_llm",
            ok,
            f"status={status} response_len={len(text)} min={MIN_RESPONSE_LEN}",
    ...
    def check_friend_chat_audio_speech_m4a(self) -> None:
        ...
        ok = status == 200 and len(text) >= MIN_RESPONSE_LEN and len(transcript) >= 2
        self._record(
            "friend_chat_audio_speech_m4a",
            ok,
            f"status={status} transcript_len={len(transcript)} response_len={len(text)} min={MIN_RESPONSE_LEN}",
            {
    ...
    def check_adb_device(self, device_id: str) -> None:
        ...
        def find_bounds_by_content_desc(ui_xml: str, content_desc: str) -> tuple[int, int, int, int] | None:
            node_regex = re.compile(
                r"<node\\b[^>]*content-desc=\"" + re.escape(content_desc) + r"\"[^>]*>",
                re.IGNORECASE,
            )
            node_match = node_regex.search(ui_xml)
            if not node_match:
                return None
            bounds_match = re.search(r'bounds="\\[(\\d+),(\\d+)\\]\\[(\\d+),(\\d+)\\]"', node_match.group(0))
            if not bounds_match:
                return None
            return tuple(int(bounds_match.group(i)) for i in range(1, 5))

        def has_content_desc(ui_xml: str, content_desc: str) -> bool:
            return find_bounds_by_content_desc(ui_xml, content_desc) is not None

        def tap_content_desc(content_desc: str) -> bool:
            ui_xml = dump_ui_xml()
            bounds = find_bounds_by_content_desc(ui_xml, content_desc)
            if bounds is None:
                return False
        ...
        # 로그인 모달이 떠 있으면 닫고 소리새 FAB 접근성 ID로 진입한다.
        ui_before = dump_ui_xml()
        login_modal_open = has_content_desc(ui_before, "worldlinco-login-modal")
        login_modal_closed = False
        if login_modal_open and tap_content_desc("worldlinco-login-close"):
            time.sleep(2)
            ui_after_close = dump_ui_xml()
            login_modal_closed = not has_content_desc(ui_after_close, "worldlinco-login-modal")

        tapped_with_accessibility_id = tap_content_desc("worldlinco-sorisae-fab")
        if not tapped_with_accessibility_id:
