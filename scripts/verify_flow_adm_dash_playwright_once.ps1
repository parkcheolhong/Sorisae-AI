...
Write-Host "[verify] FLOW-ADM-DASH playwright one-shot"
...
try {
    npx playwright test tests/admin-flow-adm-dash-regression.playwright.spec.ts --project chromium --workers=1 --reporter=list
    if ($LASTEXITCODE -ne 0) {
        throw "Playwright verification failed with exit code $LASTEXITCODE"
    }
    Write-Host "[verify] SUCCESS: FLOW-ADM-DASH refresh path regression check passed."
}
finally {
    Pop-Location
}
