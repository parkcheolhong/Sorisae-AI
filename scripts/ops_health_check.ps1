...
. (Join-Path $PSScriptRoot 'operational_domain_guard.ps1')
Assert-OperationalDomains -MarketplaceDomain $MarketplaceDomain -AdminDomain $AdminDomain -ScriptName 'scripts/ops_health_check.ps1'
...
$optionalComposeServices = @('video-worker', 'nginx')
...
foreach ($service in @('postgres', 'redis', 'qdrant', 'minio', 'backend', 'video-worker', 'frontend-admin', 'nginx')) {
	if ($availableServices -contains $service) {
		...
		$isOptional = $optionalComposeServices -contains $service
		$serviceDetail = if ($serviceInfo) {
			...
			if ($isOptional) { 'optional service not running (skipped)' } else { '서비스 정보 없음' }
		}
		$serviceOk = if ($isOptional) {
			if ($serviceInfo -eq $null) {
				$true
			}
			else {
				($serviceInfo.State -match 'running') -or ($serviceInfo.Status -match 'Up')
			}
		}
		else {
			($serviceInfo -ne $null) -and (($serviceInfo.State -match 'running') -or ($serviceInfo.Status -match 'Up'))
		}
...
if ($availableServices -contains 'nginx') {
	...
	foreach ($target in @(
			...
			@{ Name = 'ops admin ui /admin/llm'; Url = "https://$AdminDomain/admin/llm"; Expected = '200'; Host = $null; Insecure = $false },
			@{ Name = 'ops marketplace /marketplace'; Url = "https://$MarketplaceDomain/marketplace"; Expected = '200'; Host = $null; Insecure = $false }
		)) {
		try {
			Assert-AllowedOperationalUrl -Url $target.Url -ScriptName 'scripts/ops_health_check.ps1'
			$code = Get-CurlStatusCode -Url $target.Url -HostHeader $target.Host -Insecure:([bool]$target.Insecure)
			Add-Result -ResultList $results -Name $target.Name -Passed ($code -eq $target.Expected) -Detail "HTTP $code"
		}
		catch {
