...
import { buildBearerAuthHeader } from '../features/shared/authToken';
import type {
...
function mapFriendApiError(status: number, fallback: string): Error {
  if (status === 401 || status === 403) {
    return new Error('로그인이 필요합니다. 인증 상태를 확인해 주세요.');
  }
...
async function fetchWithTimeout(
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new Error('서버 응답이 지연되고 있습니다. 네트워크 상태를 확인한 뒤 다시 시도해 주세요.');
    }
    throw error;
  } finally {
...
export async function getFriends(userId: number, token: string): Promise<FriendListResponse> {
  const res = await fetchWithTimeout(`${BASE_URL}/api/users/${userId}/friends`, {
    headers: { Authorization: buildBearerAuthHeader(token) },
  });
