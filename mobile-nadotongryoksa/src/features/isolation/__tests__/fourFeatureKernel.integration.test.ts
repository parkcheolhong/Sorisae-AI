...
describe('fourFeatureRuntime shared kernel integration', () => {
    ...
    it('runs generic feature cleanup before the next feature takes over', async () => {
        const steps: string[] = [];
        registerFeatureQuiesceHandlers('face-translate', {
            stopCapture: () => {
                steps.push('capture_stop');
            ...
            cleanup: () => {
                steps.push('cleanup_timers_and_sockets');
            },
        });

        await activateFeatureExclusive('face-translate', 'test_face_start', 'user');
        const second = await activateFeatureExclusive('sorisae-ai', 'test_sorisae_start', 'user');

        expect(second.ok).toBe(true);
        expect(steps).toEqual([
            'capture_stop',
            'playback_stop',
            'cleanup_timers_and_sockets',
        ]);
        expect(getActiveFeatureId()).toBe('sorisae-ai');
    });
