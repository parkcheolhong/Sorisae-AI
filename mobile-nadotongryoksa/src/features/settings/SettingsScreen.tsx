...
import { InteractionManager, Linking, Platform, Pressable, ScrollView, StyleSheet, Switch, Text, TextInput, View } from 'react-native';
...
export function SettingsScreen({
    ...
    const previewLang = String(userLang || 'ko').trim().toLowerCase();
    const t = getSettingsText(bundledLang);
    const [selectedAlertMode, setSelectedAlertMode] = useState<IncomingAlertSoundMode>(incomingAlertSoundMode ?? 'sound');
    useEffect(() => {
        setSelectedAlertMode(incomingAlertSoundMode ?? 'sound');
    }, [incomingAlertSoundMode]);
    const settingsI18nText = useCallback((key: string, fallback: string, vars?: Record<string, string | number>) => {
    ...
    const refreshDeviceReadiness = useCallback(() => {
        ...
        void Promise.all([
            ...
            .then(([mic, location, notification, gpsEnabled]) => {
                setMicGranted(mic);
        ...
        void getCurrentVoipAudioRoute()
            .then((routeSnapshot) => {
                setAudioRoute(routeSnapshot);
            })
            .catch(() => {
                // 오디오 경로 조회 실패는 권한 상태 갱신 자체를 막지 않는다.
            });
    ...
    }, [modalSettled, refreshDeviceReadiness]);
    ...
    return (
        <View style={styles.root}>
            <View style={styles.header}>
                <Text wlLocalized style={styles.headerTitle}>{settingsI18nText('settings.headerTitle', t.headerTitle)}</Text>
                <Pressable
                    onPress={onClose}
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent}>
                <SectionCard title={settingsI18nText('settings.sectionAccount', t.sectionAccount)} accent="#0B7A4B">
                    ...
                    {hasInlineProfile ? (
                        <>
                            ...
                            >
                                <Text wlLocalized style={styles.profileRowLabel}>{settingsI18nText('settings.country', t.country)}</Text>
                            ...
                            >
                                <Text wlLocalized style={styles.profileRowLabel}>{settingsI18nText('settings.translationLanguage', t.translationLanguage)}</Text>
                                <Text wlLocalized style={styles.profileRowValue}>
                            ...
                            <Text wlLocalized style={styles.downloadLangHint}>{settingsI18nText('settings.profileLanguageLinkedHint', t.profileLanguageLinkedHint)}</Text>
                ...
                {onIncomingAlertSoundModeChange ? (
                    <SectionCard title={settingsI18nText('settings.sectionIncomingAlert', t.sectionIncomingAlert)} accent="#0B6FB0">
                        <View style={styles.segmentRow}>
                            {([
                                ...
                                const active = selectedAlertMode === item.mode;
                                return (
                                    <Pressable
                                        key={`alert-mode-${item.mode}`}
                                        style={[styles.segment, active && styles.segmentActive]}
                                        onPress={() => {
                                            setSelectedAlertMode(item.mode);
                                            onIncomingAlertSoundModeChange(item.mode);
                                        }}
                                        accessibilityRole="button"
                        ...
                        <Text style={styles.selectedAlertStatus}>
                            {`현재 수신 알림: ${({ sound: t.alertSound, vibrate: t.alertVibrate, silent: t.alertSilent })[selectedAlertMode]}`}
                        </Text>
                    </SectionCard>
                ) : null}

                <SectionCard title={settingsI18nText('settings.sectionDevicePrep', t.sectionDevicePrep)} accent="#0B7A4B">
                ...
                <SectionCard title={settingsI18nText('settings.sectionChatVoice', t.sectionChatVoice)} accent="#1E6FE0">
                    <ToggleRow
                        icon="🎙️"
                        label={t.autoListen}
                ...
                <SectionCard title={settingsI18nText('settings.sectionSorisae', t.sectionSorisae)} accent="#7A3FF2">
                    <ToggleRow
                        icon="🐦"
                        label={t.sorisaeFab}
                ...
                <View style={styles.groupCard}>
                    <ReferralInvitePanel authToken={authToken} deferLoad={modalSettled} />
                </View>

                <View style={styles.groupCard}>
                    <Text style={[styles.groupTitle, { color: '#0B6FB0' }]}>{settingsI18nText('settings.sectionManuals', t.sectionManuals)}</Text>
                    {FEATURE_MANUALS.map((m, idx) => (
                        <View key={m.id}>
                            ...
                            >
                                ...
                                <View style={styles.toggleTextWrap}>
                                    <Text style={styles.toggleLabel}>{manualPreviewTextMap[m.id]?.title ?? getBundledManual(m.id, previewLang)?.title ?? m.titleKo}</Text>
                                    <Text style={styles.toggleDesc}>{manualPreviewTextMap[m.id]?.summary ?? getBundledManual(m.id, previewLang)?.summary ?? m.summaryKo}</Text>
                                </View>
                                <Text style={styles.linkChevron}>›</Text>
                            </Pressable>
                        </View>
                ...
                {onOpenPasswordChange ? (
                    <SectionCard title={settingsI18nText('settings.sectionSecurity', t.sectionSecurity)} accent="#5f6b80">
                        <Pressable
                            style={styles.deviceActionBtn}
...
const styles = StyleSheet.create({
    ...
    segmentActive: { borderColor: '#1E6FE0', backgroundColor: '#e8f1ff', shadowColor: '#1E6FE0', shadowOpacity: 0.18, shadowRadius: 6, shadowOffset: { width: 0, height: 2 } },
