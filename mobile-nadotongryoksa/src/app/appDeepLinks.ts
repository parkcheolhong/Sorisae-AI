    ...
    try {
        ...
        if (VOIP_INCOMING_LINK_SCHEMES.includes(scheme)) {
            ...
            if (resolvedPath === APP_ENTRY_SALES_LINK_PATH) {
                const ref = String(parsed.searchParams.get('ref') || parsed.searchParams.get('code') || '').trim().toUpperCase();
...
export function parseReferralFromUrl(url: string): string | null {
    try {
        ...
        const refParam = String(parsed.searchParams.get('ref') || parsed.searchParams.get('code') || '').trim().toUpperCase();
...
export function parseAppEntryDeepLink(url: string): AppEntryDeepLinkTarget | null {
    try {
        ...
        const resolvedPath = `${parsed.hostname}${parsed.pathname}`.replace(/^\/+/, '').toLowerCase();
        if (resolvedPath === APP_ENTRY_SALES_LINK_PATH) {
            const salesAgentCode = String(parsed.searchParams.get('ref') || parsed.searchParams.get('code') || '').trim().toUpperCase();
            return salesAgentCode.startsWith('WS') ? { type: 'sales', salesAgentCode } : null;
        }
        if (resolvedPath === APP_ENTRY_INVITE_LINK_PATH) {
            const referralCode = String(parsed.searchParams.get('ref') || parsed.searchParams.get('code') || '').trim().toUpperCase();
        ...
        if (resolvedPath === APP_ENTRY_AUTH_LINK_PATH) {
            ...
            return {
                ...
                authMode: String(parsed.searchParams.get('auth_mode') || '').trim().toLowerCase() === 'passkey_login'
                    ? 'passkey_login'
                    : String(parsed.searchParams.get('auth_mode') || '').trim().toLowerCase() === 'passkey_register'
                        ? 'passkey_register'
                        : undefined,
                accessToken,
