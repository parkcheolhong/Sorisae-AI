...
/**
 * 메인 캡처 경로가 소리새로 라우팅되어야 하는지 판정한다.
 */
export function shouldRouteMainSorisaeCapture({
    songModeEnabled = false,
    sorisaeWindowOpen = false,
    faceScreenOpen = false,
}: VoiceCaptureContext = {}): boolean {
    return !songModeEnabled && Boolean(sorisaeWindowOpen) && !Boolean(faceScreenOpen);
}

/**
 * 캡처 타깃과 기능 상태를 기반으로 마이크 lease 소유 기능을 결정한다.
 */
export function resolveVoiceCaptureLeaseFeature({
    target = 'main',
    songModeEnabled = false,
    sorisaeWindowOpen = false,
    faceScreenOpen = false,
}: VoiceCaptureContext = {}): VoiceCaptureFeatureId {
    if (target === 'inter_call') {
        return 'inter_call';
    }
    if (songModeEnabled) {
        return 'song';
    }
    if (shouldRouteMainSorisaeCapture({ songModeEnabled, sorisaeWindowOpen, faceScreenOpen })) {
        return 'sorisae';
    }
    return 'face';
}
