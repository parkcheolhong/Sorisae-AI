...
const ROWS: Record<FeatureUiKey, CatalogRow> = {
    ...
    'home.toolsSub': { ko: '직접 입력·이미지(OCR) 번역을 펼쳐보세요', en: 'Expand direct input and image (OCR) translation', ja: '直接入力・画像(OCR)翻訳を開く', zh: '展开直接输入与图片(OCR)翻译' },
    ...
    'travel.nearbyRecommendSub': { ko: '현재 위치 주변의 맛집과 관광지를 추천해 드려요.', en: 'Restaurants and sights near your current location.', ja: '現在地周辺のグルメと観光スポットをおすすめします。', zh: '推荐您当前位置附近的美食与景点。' },
    'travel.searchSectionTitle': { ko: '📍 주변 검색', en: '📍 Nearby search', ja: '📍 周辺検索', zh: '📍 周边搜索' },
    'travel.searchSectionSub': { ko: '좌표/카테고리/반경을 선택해 주변 장소를 조회합니다.', en: 'Set coordinates, category, and radius to search nearby places.', ja: '座標・カテゴリ・半径を選んで周辺を検索します。', zh: '选择坐标、类别和半径以搜索周边地点。' },
    ...
    'travel.selectedPlaceMeta': { ko: '{category} · {distance} · 아래 예약 카드에 즉시 반영됩니다.', en: '{category} · {distance} · Applied to the booking form below.', ja: '{category} · {distance} · 下の予約フォームに反映されます。', zh: '{category} · {distance} · 将立即反映到下方预订卡片。' },
    ...
    'travel.bookingSectionSub': { ko: '예약 가능한 호텔/공항을 선택해 예약 요청을 진행합니다.', en: 'Select a bookable hotel or airport to start a request.', ja: '予約可能なホテル/空港を選んでリクエストします。', zh: '选择可预订的酒店/机场以提交预订请求。' },
    ...
    'voip.friendsSubtitle': { ko: '앱 가입 친구 보이스톡 · 단말 연락처와 별도', en: 'App friends VoiceTalk · separate from device contacts', ja: 'アプリ登録友達ボイストーク · 端末連絡先とは別', zh: '应用内好友语音通话 · 与手机通讯录分开' },
    ...
    'tourism.promoIntro': { ko: '여행 중 사용자가 올린 숙박·맛집 홍보를 내 언어로 표시합니다.', en: 'Shows nearby stay and dining promos in your language.', ja: '旅行中の宿泊・グルメプロモをあなたの言語で表示します。', zh: '以您的语言显示旅途中的住宿·美食推广。' },
    'tourism.translating': { ko: '번역 중...', en: 'Translating...', ja: '翻訳中...', zh: '翻译中...' },
    'tourism.gpsRequired': { ko: 'GPS로 현재 국가를 확인한 뒤 근처 숙박·음식점 홍보를 볼 수 있습니다.', en: 'Enable GPS to see nearby stay and dining promos.', ja: 'GPSで現在地を確認してから近くのプロモを表示できます。', zh: '请先通过 GPS 确认所在国家，再查看附近推广。' },
    'tourism.emptyBoard': { ko: '아직 근처 숙박·음식점 홍보가 없습니다. 여행 중 편한 곳을 첫 홍보로 올려 보세요!', en: 'No nearby promos yet. Be the first to share a place!', ja: '近くのプロモはまだありません。最初の投稿をしてみましょう！', zh: '暂无附近推广。旅途中不妨发布第一条！' },
    ...
    'sorisae.voiceCallArmOn': { ko: '📞 음성 호출 대기 켜기', en: '📞 Turn on voice-call wake', ja: '📞 音声呼び出し待機をオン', zh: '📞 开启语音呼叫等待' },
    'sorisae.voiceCallArmOff': { ko: '🔔 "{name}" 부르면 깨어나요 · 대기 끄기', en: '🔔 Say "{name}" to wake · turn off', ja: '🔔 「{name}」で起動 · 待機オフ', zh: '🔔 呼叫「{name}」唤醒 · 关闭等待' },
    'sorisae.voiceCallArmed': { ko: '🔔 음성 호출 대기 중 · "{name}" 또는 "소리새"라고 부르면 깨어나요', en: '🔔 Voice-call wake on · say "{name}" or "Sorisae"', ja: '🔔 音声呼び出し待機中 · 「{name}」または「ソリセ」', zh: '🔔 语音呼叫等待中 · 呼叫「{name}」或「索里塞」' },
    ...
    'sorisae.startConversation': { ko: '🎙️ {name} 대화 시작', en: '🎙️ Start chat with {name}', ja: '🎙️ {name}と会話開始', zh: '🎙️ 与{name}开始对话' },
    ...
    'sorisae.wakeArmedStatus': { ko: '🔔 음성 호출 대기 중 · "{name}" 또는 "소리새"라고 부르면 깨어나요', en: '🔔 Voice-call wake on · say "{name}" or "Sorisae"', ja: '🔔 音声呼び出し待機中 · 「{name}」または「ソリセ」', zh: '🔔 语音呼叫等待中 · 呼叫「{name}」或「索里塞」' },
    'sorisae.wakeEnded': { ko: '🐦 {name} 음성 호출 대기를 종료했습니다.', en: '🐦 Voice-call wake for {name} turned off.', ja: '🐦 {name}の音声呼び出し待機を終了しました。', zh: '🐦 已关闭{name}的语音呼叫等待。' },
    ...
    'capture.wakeCallWait': { ko: '🔔 호출 대기 · 조금 더 길게 불러 주세요', en: '🔔 Wake-call waiting · call a bit longer', ja: '🔔 呼び出し待機 · もう少し長く呼んでください', zh: '🔔 等待呼叫 · 请再呼叫长一点' },
    ...
    'chat.hubNearbySub': { ko: '근처 사용자 탐색과 친구 수락 흐름을 채팅 레일 안에서 이어갑니다.', en: 'Discover nearby users and accept friends within the chat rail.', ja: '近くのユーザーを探索し、チャットレール内で友達承認します。', zh: '在聊天栏内探索附近用户并完成好友接受。' },
