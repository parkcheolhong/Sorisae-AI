...
export function FriendMapDiscoveryScreen({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const permission = await getForegroundPermissions();
      const finalPermission = permission.granted ? permission : await requestForegroundPermissions();
      if (!finalPermission.granted) {
        throw new Error('위치 권한이 필요합니다.');
      }

      const position = await getCurrentPosition({ accuracy: Location.Accuracy.Balanced }, 'friend-map-balanced');
