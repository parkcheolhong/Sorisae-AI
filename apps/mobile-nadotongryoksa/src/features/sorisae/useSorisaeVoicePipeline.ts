...
} from '../settings/companionKwsSettingsStorage';
import {
...
export function useSorisaeVoicePipeline(deps: SorisaeVoicePipelineDeps) {
    ...
    useEffect(() => {
        void loadCompanionKwsSettings()
            .then((loaded) => {
                companionKwsModelPathRef.current = loaded.modelPath;
                setCompanionKwsModelPath(loaded.modelPath);
                companionKwsProviderRef.current = loaded.provider;
                setCompanionKwsProvider(loaded.provider);
                companionKwsPorcupineAccessKeyRef.current = loaded.porcupineAccessKey;
                setCompanionKwsPorcupineAccessKey(loaded.porcupineAccessKey);
                companionKwsPorcupineKeywordPathsRef.current = loaded.porcupineKeywordPaths;
                setCompanionKwsPorcupineKeywordPaths(loaded.porcupineKeywordPaths);
            })
            .catch(() => { /* no-op */ });
    }, []);
    ...
    const saveCompanionKwsSettings = useCallback(async ({
        ...
        const normalized = await persistCompanionKwsSettings({
            provider,
            modelPath,
            porcupineAccessKey,
            porcupineKeywordPaths,
        ...
        companionKwsProviderRef.current = normalized.provider;
        companionKwsModelPathRef.current = normalized.modelPath;
        companionKwsPorcupineAccessKeyRef.current = normalized.porcupineAccessKey;
        companionKwsPorcupineKeywordPathsRef.current = normalized.porcupineKeywordPaths;

        setCompanionKwsProvider(normalized.provider);
        setCompanionKwsModelPath(normalized.modelPath);
        setCompanionKwsPorcupineAccessKey(normalized.porcupineAccessKey);
        setCompanionKwsPorcupineKeywordPaths(normalized.porcupineKeywordPaths);
    ...
    const startCompanionDormantListening = useCallback(async () => {
        ...
        setGpsStatus('🔔 소리새AI 음성무한대기(심볼) 동작 중');
    }, [
        aiDisplayName,
        recordingRef,
        setAutoVoiceModeEnabled,
    ...
    const setCompanionVoiceCallArmedState = useCallback(async (nextArmed: boolean) => {
        ...
        if (!nextArmed) {
            ...
            if (!sorisaeWindowOpenRef.current) {
                setGpsStatus('🐦 소리새AI 음성무한대기(심볼)를 종료했습니다.');
            }
            return;
        }
