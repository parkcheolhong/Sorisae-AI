...
export function useVoiceCaptureLoop(deps: VoiceCaptureLoopDeps) {
    ...
    const applyVoiceCaptureBlockReason = useCallback((reason: VoiceCaptureBlockReason | null) => {
        setVoiceCaptureBlockReason(reason);
        if (reason) {
            console.log('[FACE_CAPTURE_BLOCK]', JSON.stringify({
                code: reason.code,
                message: reason.message,
                trace_id: reason.traceId ?? null,
                from_lang: fromLang,
                to_lang: toLang,
            }));
        }
    }, [fromLang, setVoiceCaptureBlockReason, toLang]);

    const logFaceCaptureTrace = useCallback((event: string, data: Record<string, unknown> = {}) => {
        const traceId = faceCaptureTraceIdRef.current;
        if (!traceId) {
            return;
        }
        console.log('[FACE_CAPTURE_TRACE]', JSON.stringify({
            trace_id: traceId,
            event,
            ...data,
        }));
    ...
    const startVoiceInput = useCallback(async (options: { autoMode?: boolean; target?: 'main' | 'inter_call' } = {}) => {
        ...
        if (inputTarget === 'main' && effectiveAutoMode) {
            ...
            logFaceCaptureTrace('start_tap', {
                from_lang: fromLang,
                to_lang: toLang,
                face_screen_open: faceScreenOpenRef.current,
                sorisae_window_open: sorisaeWindowOpenRef.current,
            });
        }
        applyVoiceCaptureBlockReason(null);
        try {
            ...
            if (
                ...
                const message = getDisplayUiText().faceConversationPeerRequired
                    ?? '상대 언어를 GPS 또는 수동 선택으로 지정해 주세요.';
                applyVoiceCaptureBlockReason({
                    code: 'peer_language_required',
                    message,
                    traceId: faceCaptureTraceIdRef.current ?? undefined,
                });
                setGpsStatus(message);
                logFaceCaptureTrace('blocked_peer_language_required', {
                    from_lang: fromLang,
                    to_lang: toLang,
                });
                return;
            ...
            if (isVoipSessionActive()) {
                ...
                applyVoiceCaptureBlockReason({
                    code: 'capture_blocked_voip_session',
                    message: 'VoIP 통화 세션이 활성화되어 대면 통역 캡처를 시작할 수 없습니다.',
                    traceId: faceCaptureTraceIdRef.current ?? undefined,
                });
                logFaceCaptureTrace('blocked_voip_session');
            ...
            if (
                ...
                applyVoiceCaptureBlockReason({
                    code: 'capture_blocked_playback_window',
                    message: '직전 음성 출력 잔향 보호 구간입니다. 잠시 후 자동 재시작됩니다.',
                    traceId: faceCaptureTraceIdRef.current ?? undefined,
                });
                logFaceCaptureTrace('blocked_playback_window', {
                    blocked_until_ms: sorisaePlaybackBlockedUntilRef.current,
                });
                scheduleFaceConversationRestartRef.current(null);
                return;
            ...
            if (allowFaceDuplexCapture) {
                ...
                logFaceCaptureTrace('blocked_speaking', {
                    face_playback: interruptedFacePlayback,
                    sorisae_playback: interruptedSorisaePlayback,
                });
                scheduleFaceConversationRestartRef.current(null);
            ...
            if (Platform.OS === 'web') {
                ...
                if (!speechCtor) {
                    ...
                    applyVoiceCaptureBlockReason({
                        code: 'capture_web_unsupported',
                        message: getFeatureUiText('capture.micWebUnsupportedBody'),
                        traceId: faceCaptureTraceIdRef.current ?? undefined,
                    });
                    return;
                }
            ...
            if (!hasPermission) {
                applyVoiceCaptureBlockReason({
                    ...
                    message: '마이크 권한이 없어 음성 캡처를 시작할 수 없습니다.',
                    traceId: faceCaptureTraceIdRef.current ?? undefined,
                });
                logFaceCaptureTrace('blocked_permission_denied');
            ...
            if (inputTarget === 'main' && effectiveAutoMode) {
                logFaceCaptureTrace('capture_audio_mode_begin', {
                    stage: 'before_set_audio_mode',
                });
            }
            ...
            const isAutoConversationCapture = effectiveAutoMode && inputTarget === 'main'
                && (isFaceConversation || isCompanionVoiceCallCapture);
            ...
            if (effectiveAutoMode) {
                try {
                    ...
                    if (inputTarget === 'main') {
                        logFaceCaptureTrace('capture_audio_mode_error', {
                            enabled: false,
                        });
                    }
            ...
            if (inputTarget === 'main' && effectiveAutoMode) {
                logFaceCaptureTrace('capture_create_begin', {
                    is_auto_conversation_capture: isAutoConversationCapture,
                    is_face_conversation: isFaceConversation,
            ...
            if (inputTarget === 'main' && effectiveAutoMode) {
                logFaceCaptureTrace('capture_started');
            }
            if (inputTarget === 'main' && effectiveAutoMode) {
            ...
            if (inputTarget === 'main') {
                ...
                mainSorisaeRouteRef.current = shouldRouteMainSorisaeCapture({
                    songModeEnabled,
            ...
            {
                const leaseFeature = resolveVoiceCaptureLeaseFeature({
                    target: inputTarget,
                    songModeEnabled,
                    sorisaeWindowOpen: sorisaeWindowOpenRef.current,
                    faceScreenOpen: faceScreenOpenRef.current,
            ...
            if (effectiveAutoMode) {
                ...
                if (inputTarget === 'inter_call') {
                ...
                } else if (isAutoConversationCapture) {
                    setGpsStatus(
                    ...
                    const sorisaeVadConfig = sorisaeWindowOpenRef.current && mainSorisaeRouteRef.current
                        ...
                        : undefined;
                    const companionKwsInitPath = Boolean(inputTarget === 'main' && companionVoiceCallArmedRef.current);
                    const companionKwsRouteReady = Boolean(sorisaeWindowOpenRef.current && mainSorisaeRouteRef.current);
                    if (!companionKwsInitPath) {
                        console.log('[COMPANION_HANDLER]', JSON.stringify({
                            event: 'COMPANION_KWS_INIT_SKIP',
                            reason: inputTarget !== 'main' ? 'non_main_target' : 'not_armed',
                            companion_armed: companionVoiceCallArmedRef.current,
                            companion_phase: companionVoiceCallRef.current.phase,
                            route_ready: companionKwsRouteReady,
                            target: inputTarget,
                        }));
                    }
                    if (companionKwsInitPath) {
                        console.log('[COMPANION_HANDLER]', JSON.stringify({
                            event: 'COMPANION_KWS_INIT_BEGIN',
                            provider: 'silero_vad',
                            companion_armed: companionVoiceCallArmedRef.current,
                            companion_phase: companionVoiceCallRef.current.phase,
                            route_ready: companionKwsRouteReady,
                            has_custom_vad_config: Boolean(sorisaeVadConfig),
                        }));
                    }
                    ...
                    if (companionKwsInitPath) {
                        console.log('[COMPANION_HANDLER]', JSON.stringify({
                            event: 'COMPANION_KWS_INIT_END',
                            provider: 'silero_vad',
                            stage: 'face_vad_started',
                            route_ready: companionKwsRouteReady,
                        }));
                    }
                        ...
                        try {
                            ...
                            if (companionKwsInitPath) {
                                console.log('[COMPANION_HANDLER]', JSON.stringify({
                                    event: 'COMPANION_KWS_INIT_END',
                                    provider: 'silero_vad',
                                    stage: 'native_vad_monitor_ready',
                                    monitor_started: monitorStarted,
                                    capture_active: faceSileroCaptureActiveRef.current,
                                    route_ready: companionKwsRouteReady,
                                }));
                            }
                        } catch {
                            ...
                            if (companionKwsInitPath) {
                                console.log('[COMPANION_HANDLER]', JSON.stringify({
                                    event: 'COMPANION_KWS_INIT_ERROR',
                                    provider: 'silero_vad',
                                    stage: 'native_vad_monitor_start',
                                    route_ready: companionKwsRouteReady,
                                }));
                            }
                        }
            ...
            if (inputTarget === 'main' && companionVoiceCallArmedRef.current) {
                console.log('[COMPANION_HANDLER]', JSON.stringify({
                    event: 'COMPANION_KWS_INIT_ERROR',
                    provider: 'silero_vad',
                    stage: 'start_voice_input_catch',
                    route_ready: Boolean(sorisaeWindowOpenRef.current && mainSorisaeRouteRef.current),
                    detail,
                }));
            }
            setIsVoiceRecording(false);
            setVoiceSttLoading(false);
            const activeLeaseOwner = currentVoiceCaptureOwner();
            if (activeLeaseOwner && recordingRef.current) {
                await stopVoiceInputRef.current?.({ suppressAutoRestart: true, discardSegment: true });
            } else if (activeLeaseOwner) {
                releaseVoiceCapture(activeLeaseOwner);
            }
            setGpsStatus(getFeatureUiText('capture.voiceInputFailed', { detail }));
            applyVoiceCaptureBlockReason({
                code: 'capture_start_error',
                message: getFeatureUiText('capture.voiceInputFailed', { detail }),
                traceId: faceCaptureTraceIdRef.current ?? undefined,
            });
            logFaceCaptureTrace('capture_start_error', { detail });
            if (effectiveAutoMode && autoVoiceModeEnabledRef.current && inputTarget === 'main') {
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            // stop-in-flight가 장시간 고착되면 startVoiceInput이 영구 차단될 수 있어
            // 녹음/로딩이 모두 꺼진 상태에서만 안전하게 가드를 해제한다.
            if (voiceInputStopInFlightRef.current
                && !recordingRef.current
                && !voiceSttLoadingRef.current) {
                if (stopInFlightStartedAtMsRef.current <= 0) {
                    stopInFlightStartedAtMsRef.current = Date.now();
                } else if (Date.now() - stopInFlightStartedAtMsRef.current >= 6000) {
                    console.log('[FACE_CONVERSATION]', JSON.stringify({ event: 'stop_inflight_stale_recover' }));
    ...
    const stopVoiceInput = useCallback(async (options: { suppressAutoRestart?: boolean; discardSegment?: boolean } = {}) => {
        ...
        try {
            ...
            if (!uri) {
                applyVoiceCaptureBlockReason({
                    code: 'capture_uri_missing',
                    message: '녹음 세그먼트를 찾지 못해 이번 턴을 건너뜁니다.',
                    traceId: faceCaptureTraceIdRef.current ?? undefined,
                });
                logFaceCaptureTrace('capture_uri_missing');
                scheduleFaceConversationRestartRef.current(null);
                return;
            }
            if (isVoipSessionActive()) {
                ...
                applyVoiceCaptureBlockReason({
                    code: 'upload_blocked_voip_session',
                    message: 'VoIP 세션 중이라 음성 업로드가 차단되었습니다.',
                    traceId: faceCaptureTraceIdRef.current ?? undefined,
                });
                logFaceCaptureTrace('blocked_upload_voip_session');
                scheduleFaceConversationRestartRef.current(null);
                return;
            }
            let uploadUri = uri;
                ...
                try {
                    ...
                    if (capture && capture.byteCount > 0) {
                        ...
                        if (!faceSileroHadSpeechSnapshot
                            ...
                            // 창이 열려 있으면 소리새 질문으로 보고 m4a 폴백을 유지한다.
                            // mainSorisaeRouteRef는 캡처 시점 스냅샷이라 UI 전환 경계에서 일시 false가 될 수 있다.
                            const sorisaeM4aFallback = sorisaeWindowOpenRef.current;
            ...
            try {
                ...
                if (autoVoiceModeEnabledRef.current
                    ...
                    if (allowFaceBargeIn) {
                        ...
                        applyVoiceCaptureBlockReason({
                            code: 'segment_discard_while_speaking',
                            message: getFeatureUiText('capture.echoIgnored'),
                            traceId: faceCaptureTraceIdRef.current ?? undefined,
                        });
                        logFaceCaptureTrace('segment_discard_while_speaking');
                        return;
                    }
                }
                ...
                logFaceCaptureTrace('payload_prepared', {
                    endpoint: voiceEndpoint,
                    mode: songModeEnabled ? 'song' : (isFaceGptMode ? 'friend_chat' : (autoVoiceModeEnabled ? 'bilingual' : 'designated')),
                    from_lang: localInterpretLang,
                    to_lang: effectiveInterpretToLang,
                    upload_kind: uploadUri === uri ? 'm4a' : 'silero_wav',
                });
                ...
                if (isFaceGptMode) {
                    const gpsServicesEnabled = await getLocationServicesEnabled().catch(() => true);
                ...
                if (res.ok) {
                    ...
                    applyVoiceCaptureBlockReason(null);
                    logFaceCaptureTrace('response_received', {
                        status: res.status,
                        ok: true,
                    });
            ...
            if (autoVoiceModeEnabledRef.current && voiceInputTargetRef.current === 'main') {
                ...
                logFaceCaptureTrace('segment_error', { message });
                if (!options.suppressAutoRestart) {
                    scheduleFaceConversationRestartRef.current(null);
                }
            }
