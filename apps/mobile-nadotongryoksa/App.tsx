...
import { useSorisaeConversationController } from './src/features/sorisae/sorisaeConversationController';
import { buildCompanionVoiceCallControllerDeps, useCompanionVoiceCallController } from './src/features/sorisae/companionVoiceCallController';
import { useSorisaeDormantWatchdog } from './src/features/sorisae/sorisaeDormantWatchdog';
import { syncWakeCompanionVoiceCallNowRef, useSorisaeWakeController } from './src/features/sorisae/sorisaeWakeController';
import { useSorisaeWindowController, useSorisaeWindowOpenStateController } from './src/features/sorisae/sorisaeWindowController';
import {
...
import { buildFaceConversationControllerDeps, useFaceConversationAutoVoiceGuards, useFaceConversationController } from './src/features/face-interpretation/faceConversationController';
import { FaceInterpretationScreen } from './src/features/face-interpretation/FaceInterpretationScreen';
...
import { applyBookingRequestFailureNow, applyBookingRequestSuccessNow, applyNearbySearchFailureNow, applyNearbySearchSuccessNow, beginBookingRequestNow, beginNearbySearchNow, finishBookingRequestNow } from './src/features/travel-booking/travelBookingController';
...
import {
    SOCIAL_LOGIN_PROVIDER_CONFIGS,
    openSocialLoginProvider,
    type SocialLoginProvider,
} from './src/auth/socialLogin';
...
import { syncRefCurrent } from './src/features/shared/refSync';
import { beginSorisaeButtonDrag, markSorisaeButtonDragged, openSorisaeWindowIfTap } from './src/features/sorisae/sorisaeOpenButton';
import { applySongFileTimelineReadyNow, beginSongFileSelection, exportSongFileTimelineNow, loadSongFileSoundNow, replaceSongFileSegment, resetSongFileWorkspaceNow, toggleSongFilePlaybackNow, updateSongFileSegmentText } from './src/features/song/songFilePlaybackController';
import {
    buildVoiceId,
    buildVoipTopic,
...
(() => {
    const installWrapper = (key: 'Text' | 'TextInput', mode: 'text' | 'input') => {
        ...
        const Wrapped: any = function WlTextWrapper(props: any) {
            useUiI18nTick();
            const { wlLocalized, noI18n, children, placeholder, accessibilityLabel, style, ...restProps } = props;
            const skipI18n = Boolean(wlLocalized || noI18n);
            const uiLang = getEffectiveUiLang();
            let nextChildren = children;
            let nextPlaceholder = placeholder;
            let nextAccessibilityLabel = accessibilityLabel;
            if (!skipI18n && uiLang !== 'ko') {
                if (mode === 'text' && nextChildren != null) {
                    nextChildren = translateChildrenDeep(nextChildren);
                }
                if (mode === 'input' && typeof nextPlaceholder === 'string') {
                    nextPlaceholder = translateUiSync(nextPlaceholder);
                }
                if (typeof nextAccessibilityLabel === 'string') {
                    nextAccessibilityLabel = translateUiSync(nextAccessibilityLabel);
                }
            }
            const flat = StyleSheet.flatten(style) as { fontSize?: number } | undefined;
            const nextStyle = flat && typeof flat.fontSize === 'number'
                ? [style, { fontSize: Math.round(flat.fontSize * GLOBAL_FONT_SCALE) }]
                : style;
...
function buildInstantDemoCredentials(seed: string) {
    ...
    const rawDemoDomain = String(DEMO_SESSION_EMAIL_DOMAIN || '').trim().toLowerCase();
    const demoDomain = rawDemoDomain.includes('.') ? rawDemoDomain : `${rawDemoDomain || 'worldlinco-demo'}.com`;
    return {
        email: `instant-${normalizedSeed}@${demoDomain}`,
...
async function runApkInAppInstall(apiBase: string) {
    ...
    let lastShown = 0;
    const result = await downloadAndInstallLatestApk(apiBase, {
        onProgress: (ratio) => {
...
async function checkForAppUpdate(apiBase: string) {
    try {
        ...
        const metadata = await fetchLatestApkMetadata(apiBase);
        if (!metadata) {
            return;
        }
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    ...
                    style: 'default',
                    onPress: () => {
                        runApkInAppInstall(apiBase).catch((err) =>
...
async function callSignupApi(apiBase: string, payload: SignupPayload): Promise<UserInfo> {
...
async function callSignupRequestCodeApi(apiBase: string, payload: SignupPayload): Promise<SignupRequestCodeResponse> {
    const res = await fetch(`${apiBase}/api/auth/signup/request-code`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            ...payload,
...
async function callSignupConfirmApi(
    ...
    const res = await fetch(`${apiBase}/api/auth/signup/confirm`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            signupSessionToken,
...
async function callMeApi(apiBase: string, token: string): Promise<UserInfo> {
    ...
    if (!res.ok) throw new Error(`내 정보 조회 실패 (HTTP ${res.status})`);
    return res.json();
}

async function callUpdateMeApi(apiBase: string, token: string, payload: UserProfileUpdatePayload): Promise<UserInfo> {
    const res = await fetch(`${apiBase}/api/auth/me`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json',
...
async function callNearbyPlacesApi(apiBase: string, params: {
    lat: string;
    lon: string;
    ...
    const requestUrl = `${apiBase}/api/marketplace/nadotongryoksa/lbs/nearby?${query.toString()}`;
...
async function callBookingApi(apiBase: string, token: string, payload: {
    placeId: string;
    customerName: string;
    checkinDate: string;
    checkoutDate: string;
    ...
    const response = await fetch(`${apiBase}/api/marketplace/nadotongryoksa/lbs/bookings`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
...
async function callCreatePurchaseApi(apiBase: string, token: string, amount: number): Promise<PurchaseResult> {
    const projectId = await resolveWorldLincoProjectId(apiBase);
...
async function callInitiatePaymentApi(apiBase: string, token: string, purchaseId: number): Promise<{ payment_url: string }> {
...
async function callMyPurchasesApi(apiBase: string, token: string): Promise<Array<{ id: number; amount: number; status: string; payment_method: string }>> {
    const res = await fetch(`${apiBase}/api/marketplace/purchases`, {
        headers: { Authorization: `Bearer ${token}` },
    });
...
async function callCreateSongFileJob(apiBase: string, asset: DocumentPicker.DocumentPickerAsset, targetLanguage: LangCode): Promise<SongFileJobStatus> {
    ...
    const response = await fetch(`${apiBase}/api/mobile/song-translation/jobs`, {
        method: 'POST',
        body: formData,
    });
...
async function callSongFileJobStatus(apiBase: string, jobId: string): Promise<SongFileJobStatus> {
    // ===== POLLING TIMING =====
    const pollStartTime = Date.now();
    const response = await fetch(`${apiBase}/api/mobile/song-translation/jobs/${encodeURIComponent(jobId)}`);
...
async function callSongFileTimeline(apiBase: string, jobId: string): Promise<SongFileTimeline> {
    const response = await fetch(`${apiBase}/api/mobile/song-translation/jobs/${encodeURIComponent(jobId)}/subtitles`);
    return parseApiResponse<SongFileTimeline>(response);
}

async function callPatchSongFileSegment(apiBase: string, jobId: string, segmentId: string, translated: string): Promise<SongFileTimelineSegment> {
    const response = await fetch(`${apiBase}/api/mobile/song-translation/jobs/${encodeURIComponent(jobId)}/segments/${encodeURIComponent(segmentId)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
async function callExportSongFileTimeline(apiBase: string, jobId: string, format: 'srt' | 'vtt' | 'lrc' | 'json'): Promise<string> {
    const query = new URLSearchParams({ format });
    const response = await fetch(`${apiBase}/api/mobile/song-translation/jobs/${encodeURIComponent(jobId)}/export?${query.toString()}`);
    const text = await response.text();
    if (!response.ok) throw new Error(text || `HTTP ${response.status}`);
    return text;
}

async function callCreateVoiceConsent(apiBase: string): Promise<VoiceConsentResponse> {
    const response = await fetch(`${apiBase}/api/mobile/song-translation/voice-consents`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
async function callCreateVoiceProfile(apiBase: string, asset: DocumentPicker.DocumentPickerAsset, consentId: string): Promise<VoiceProfileResponse> {
    const formData = new FormData();
    ...
    const response = await fetch(`${apiBase}/api/mobile/song-translation/voice-profiles`, {
        method: 'POST',
        body: formData,
    });
...
async function callDeleteVoiceProfile(apiBase: string, profileId: string): Promise<void> {
    const response = await fetch(`${apiBase}/api/mobile/song-translation/voice-profiles/${encodeURIComponent(profileId)}`, {
        method: 'DELETE',
    });
    await parseApiResponse<{ deleted: boolean }>(response);
}

async function callCreateVoicePreview(apiBase: string, params: {
    jobId: string;
    ...
    const response = await fetch(`${apiBase}/api/mobile/song-translation/jobs/${encodeURIComponent(params.jobId)}/voice-preview`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
function AppInner() {
    const { apiBase, auth, appUi, profile } = useAppRuntime();
    ...
    useEffect(() => {
        syncRefCurrent(peerLangManualRef, peerLangManual);
    }, [peerLangManual]);
    const toLangRef = useRef<LangCode>(toLang);
    useEffect(() => {
        syncRefCurrent(toLangRef, toLang);
    }, [toLang]);
    ...
    const lastSuccessfulAuthDeepLinkUrlRef = useRef('');
    const authDeepLinkInFlightRef = useRef(false);
    ...
    const { initiateVoipCall, validatePhoneNumber } = useVoipAutoController(apiBase, token);
    ...
    const handleBiometricLogin = useCallback(async () => {
        ...
        try {
            ...
            const tk = await callLoginApi(apiBase, credentials.email, credentials.password);
            const me = await callMeApi(apiBase, tk);
        ...
        try {
            ...
            for (let attempt = 0; attempt < 2; attempt += 1) {
                ...
                try {
                    ...
                    const nextToken = await callLoginApi(apiBase, demoCreds.email, demoCreds.password);
                    const nextUserInfo = await callMeApi(apiBase, nextToken);
                    console.log('[AUTH_FLOW]', JSON.stringify({
    ...
    useEffect(() => { syncRefCurrent(aiDisplayNameRef, aiDisplayName); }, [aiDisplayName]);
    /**
    ...
    useEffect(() => { syncRefCurrent(companionVoiceCallArmedRef, companionVoiceCallArmed); }, [companionVoiceCallArmed]);
    ...
    const sorisaePanResponder = useRef(
        PanResponder.create({
            ...
            onPanResponderGrant: () => {
                beginSorisaeButtonDrag({ dragMovedRef: sorisaeDragMovedRef, buttonPosition: sorisaeBtnPos });
            },
            onPanResponderMove: (e, g) => {
                markSorisaeButtonDragged({ dragMovedRef: sorisaeDragMovedRef }, g.dx, g.dy);
                Animated.event(
            ...
            onPanResponderRelease: () => {
                ...
                openSorisaeWindowIfTap({ dragMovedRef: sorisaeDragMovedRef, sorisaeWindowOpenRef, setSorisaeWindowOpen });
            },
        }),
    ).current;
    ...
    const handleSettingsChangeCountry = useCallback((code: string) => {
        ...
        void (async () => {
            try {
                ...
                const updated = await callUpdateMeApi(apiBase, token, payload);
                setUserInfo(updated);
    ...
    const handleSettingsChangeLanguage = useCallback((code: string) => {
        ...
        void (async () => {
            try {
                const updated = await callUpdateMeApi(apiBase, token, {
                    preferred_language: pair.languageCode,
                    country_code: pair.countryCode,
                });
                setUserInfo(updated);
    ...
    const applyIncomingVoipPayload = useCallback((payload: Partial<CallInitResponse> & { caller_label?: string; caller_voice_id?: string }, source: string) => {
        ...
        if (
            ...
            if (
                ...
                void requestEndVoipCall(apiBase, token, normalizedPayload.call_id, 'stale_non_ring_session');
            }
            return;
        }
    ...
    }, [apiBase, logUiPressProbe, populateIncomingVoipPresentation, startIncomingVoipAlert, stopIncomingVoipAlert, summarizeIncomingVoipPayload, token, userInfo]);
    ...
    const handleOpenChatRoomById = useCallback(async (roomId: string, source: string) => {
        ...
        try {
            const detail = await getChatRoomDetail(apiBase, token, roomId);
    ...
    }, [apiBase, handleOpenChatRoom, logUiPressProbe, token, userInfo]);
    ...
    const autoAcceptIncomingVoipDeepLink = useCallback(async (
        ...
        if (token) {
            try {
                const acceptedFromServer = await acceptIncomingCall(apiBase, token, payload.call_id);
                const callerLanguageHint = resolveVoipRemoteLanguageHint(
                ...
                const snapshot = await fetchVoipCallResumeSnapshot(apiBase, token, payload.call_id);
    ...
    }, [apiBase, activateAcceptedIncomingVoipCall, applyIncomingVoipPayload, logUiPressProbe, requestPermissions, setIncomingVoipAcceptInFlight, stopIncomingVoipAlert, token]);
    ...
    const resolveStalePendingIncomingCall = useCallback(async (
        ...
        const snapshot = await fetchVoipCallResumeSnapshot(apiBase, token, localPending.call_id);
        ...
        if (snapshot?.call_id === localPending.call_id && snapshot.status === 'connecting') {
            await requestEndVoipCall(apiBase, token, localPending.call_id, 'stale_pending_connecting');
            stopIncomingVoipAlert(`${source}:${reason}`);
            setPendingIncomingVoipCall(null);
        ...
        return true;
    }, [apiBase, dismissPendingIncomingAsMissed, logUiPressProbe, stopIncomingVoipAlert, token]);
    ...
    const fetchPendingIncomingVoipCall = useCallback(async (source: string) => {
        ...
        try {
            ...
            const response = await fetch(`${apiBase}/api/v1/voip/calls/pending-incoming`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
    ...
    }, [apiBase, applyIncomingVoipPayload, buildVoipRemoteProfile, dismissPendingIncomingAsMissed, logUiPressProbe, resolveStalePendingIncomingCall, token, userInfo, voipCallInitResponse]);
    ...
    const refreshVoipAudit = useCallback(async (callId: string, options?: { showLoading?: boolean; force?: boolean }) => {
        ...
        setVoipAuditError('');
        try {
            const response = await fetch(`${apiBase}/api/v1/voip/calls/${callId}/audit`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
    ...
    useEffect(() => {
        checkForAppUpdate(apiBase).catch((err) => console.error('앱 버전 체크 오류:', err));
        ...
        const sub = AppState.addEventListener('change', (state) => {
            ...
            checkForAppUpdate(apiBase).catch((err) => console.error('앱 버전 체크 오류(foreground):', err));
        });
        return () => sub.remove();
    }, []);

    useEffect(() => {
        console.log('[WORLDLINGCO_API] base_url', apiBase);
        void hydrateWorldlincoTuningFromStorage()
            .then(() => refreshWorldlincoTuning(apiBase))
            .catch((err) => console.error('[WORLDLINGCO_TUNING] bootstrap failed', err));
        const sub = AppState.addEventListener('change', (state) => {
            if (state === 'active') {
                void refreshWorldlincoTuning(apiBase);
            }
        });
        return () => sub.remove();
    }, []);
        ...
        (async () => {
            try {
                ...
                if (!cancelled && storedAuth) {
                    ...
                    try {
                        const me = await callMeApi(apiBase, storedAuth.token);
                        if (!cancelled) {
                            console.log('[AUTH_FLOW]', JSON.stringify({
    ...
    useEffect(() => {
        ...
        const syncVoipTopic = async () => {
            ...
            try {
                ...
                if (pushToken && token) {
                    await registerVoipDevice(apiBase, token, pushToken);
                }
                if (!cancelled) {
    ...
    useEffect(() => {
        ...
        const announceMissedCalls = async () => {
            try {
                const missed = await fetchRecentMissedCalls(apiBase, token);
        ...
        autoVoiceTranslationTimerRef.current = setTimeout(async () => {
            try {
                ...
                if (canUseVoicePreview && songFileJob && voiceProfile) {
                    ...
                    const preview = await callCreateVoicePreview(apiBase, {
    ...
    const loadSongFileSound = useCallback(async (asset: DocumentPicker.DocumentPickerAsset) => {
        await loadSongFileSoundNow({
            ...
            createSoundAsync: async (uri, onStatus) => {
                const { sound } = await Audio.Sound.createAsync(
                    { uri },
                    { shouldPlay: false },
                    onStatus,
    ...
    const handlePickSongFile = useCallback(async () => {
        ...
        try {
            ...
            const createdJob = await callCreateSongFileJob(apiBase, asset, fileTargetLang);
            ...
            while (Date.now() - pollStartedAt < SONG_FILE_JOB_MAX_WAIT_MS) {
                ...
                latestJob = await callSongFileJobStatus(apiBase, createdJob.job_id);
            ...
            const timeline = await callSongFileTimeline(apiBase, createdJob.job_id);
            ...
            applySongFileTimelineReadyNow(
                ...
                timeline.segments,
                getLangLabelText(detectedSource),
                getLangLabelText(detectedTarget),
                timeline.segment_count,
                timeline.quality_score,
            );
        } catch (error) {
    ...
    const handleToggleSongFilePlayback = useCallback(async () => {
        await toggleSongFilePlaybackNow(songFileSoundRef, Alert.alert);
    }, []);

    const handleSongFileSegmentTextChange = useCallback((segmentId: string, translated: string) => {
        setSongFileSegments((prev) => updateSongFileSegmentText(prev, segmentId, translated));
    }, []);

    const handleSaveSongFileSegment = useCallback(async (segment: SongFileTimelineSegment) => {
        if (!songFileJob) return;
        try {
            const updatedSegment = await callPatchSongFileSegment(apiBase, songFileJob.job_id, segment.id, segment.translated);
            setSongFileSegments((prev) => replaceSongFileSegment(prev, updatedSegment));
    ...
    const handleExportSongFileTimeline = useCallback(async (format: 'srt' | 'vtt' | 'lrc' | 'json') => {
        ...
        try {
            await exportSongFileTimelineNow({
                setSongFileExportPreview,
                setSongModeStatus,
            }, songFileJob.job_id, format, (jobId, exportFormat) => callExportSongFileTimeline(apiBase, jobId, exportFormat));
        } catch (error) {
    ...
    const ensureVoiceConsent = useCallback(async (): Promise<VoiceConsentResponse> => {
        ...
        const createdConsent = await callCreateVoiceConsent(apiBase);
    ...
    const handlePickVoiceSample = useCallback(async () => {
        ...
        try {
            ...
            const createdProfile = await callCreateVoiceProfile(apiBase, picked.assets[0], consent.consent_id);
    ...
    const handleToggleVoiceSampleRecording = useCallback(async () => {
        ...
        try {
            ...
            const consent = await ensureVoiceConsent();
            const createdProfile = await callCreateVoiceProfile(apiBase, {
                uri,
    ...
    const handleDeleteVoiceProfile = useCallback(async () => {
        ...
        setVoiceProfileLoading(true);
        try {
            await callDeleteVoiceProfile(apiBase, voiceProfile.voice_profile_id);
    ...
    const handleLogin = useCallback(async () => {
        ...
        setLoginError('');
        try {
            const tk = await callLoginApi(apiBase, loginEmail.trim(), loginPw);
            const me = await callMeApi(apiBase, tk);
            console.log('[AUTH_FLOW]', JSON.stringify({
    ...
    const handleSocialLoginStart = useCallback(async (provider: SocialLoginProvider) => {
        setLoginError('');
        try {
            await openSocialLoginProvider(provider, apiBase);
        } catch (error: any) {
    ...
    const handleSocialAuthCallback = useCallback(async (
        target: Extract<AppEntryDeepLinkTarget, { type: 'auth' }>,
        source: string,
    ): Promise<boolean> => {
        const accessToken = target.accessToken.trim();
        const authMode = target.authMode ?? null;
        const isPasskeyLogin = authMode === 'passkey_login';
        ...
        if (!accessToken) {
            const emptyTokenMessage = '소셜 로그인 토큰이 비어 있습니다.';
            setLoginError(emptyTokenMessage);
            if (isPasskeyLogin) {
                logUiPressProbe('PASSKEY_LOGIN_CALLBACK_FAIL', {
                    source,
                    auth_mode: authMode,
                    error: emptyTokenMessage,
                });
            }
        ...
        try {
            const me = await callMeApi(apiBase, accessToken);
            applyAuthenticatedSession(accessToken, me);
            setDemoSessionMessage('');
            setAuthModalMode('login');
            setShowLogin(false);

            const postAuthSorisaeFabVisible = Platform.OS !== 'web'
                && Boolean(me)
                && !voipCallInitResponse
                && !pendingIncomingVoipCall
                && !sorisaeWindowOpen
                && globalSettings.sorisaeFab;
            logUiPressProbe('SORISAE_FAB_VISIBLE_EVAL', {
                visible: postAuthSorisaeFabVisible,
                source,
                auth_mode: authMode,
                reason: 'post_auth_callback',
                sorisae_fab_setting: globalSettings.sorisaeFab,
                pending_incoming_voip: Boolean(pendingIncomingVoipCall),
                voip_call_active: Boolean(voipCallInitResponse),
                sorisae_window_open: sorisaeWindowOpen,
            });

            if (isPasskeyLogin) {
                const successMessage = '패스키 로그인 콜백 성공';
                console.log('[AUTH_FLOW]', JSON.stringify({
                    event: 'PASSKEY_LOGIN_CALLBACK_SUCCESS',
                    auth_mode: authMode,
                    success_message: `${successMessage} (auth_mode=${authMode})`,
                    source,
                    user_id: me.id,
                    user_email: me.email,
                }));
                logUiPressProbe('PASSKEY_LOGIN_CALLBACK_SUCCESS', {
                    source,
                    auth_mode: authMode,
                    success_message: '패스키 로그인 성공',
                    user_id: me.id,
                    user_email: me.email,
                });
            }
            ...
            const errorMessage = error?.message || '소셜 로그인 복원 실패';
    ...
    const handleSignupRequestCode = useCallback(async () => {
        ...
        setLoginError('');
        try {
            const response = await callSignupRequestCodeApi(apiBase, {
    ...
    const handleSignupConfirm = useCallback(async () => {
        ...
        setLoginError('');
        try {
            const signedUpUser = await callSignupConfirmApi(apiBase, signupSessionToken, trimmedOtp, {
                preferred_language: preferredLanguage,
                country_code: normalizedCountryCode || undefined,
                full_name: signupFullName.trim() || undefined,
            });
            const tk = await callLoginApi(apiBase, normalizedEmail, normalizedPassword);
            const me = await callMeApi(apiBase, tk);
    ...
    const handleDispatchChatShare = useCallback(async (room: ChatRoomSummary, params: {
        ...
        try {
            await sendChatRoomMessage(apiBase, token, room.room_id, {
    ...
    const handleShareMessageToChat = useCallback(async (params: {
        ...
        try {
            const [selfRoom, recentRooms] = await Promise.all([
                ensureSelfChatRoom(apiBase, token),
                listChatRooms(apiBase, token),
            ]);
        ...
        void (async () => {
            try {
                ...
                if (pushToken && logoutToken) {
                    await unregisterVoipDevice(apiBase, logoutToken, pushToken);
                }
                if (logoutTopic) {
    ...
    useEffect(() => {
        ...
        const connectPresence = (reason: string) => {
            ...
            const url = buildVoipWebSocketUrl(apiBase, '/api/v1/voip/presence', { token });
    ...
    const handleShowPurchases = useCallback(async () => {
        ...
        try {
            const list = await callMyPurchasesApi(apiBase, token);
            setMyPurchases(list);
        } catch {
            setMyPurchases([]);
        } finally {
    ...
    const handlePremiumPurchase = useCallback(async (planKey: MonetizationPlanKey) => {
        ...
        try {
            const purchase = await callCreatePurchaseApi(apiBase, token, plan.amount);
            setPurchaseResult(purchase);
            const payData = await callInitiatePaymentApi(apiBase, token, purchase.id);
    ...
    useEffect(() => {
        ...
        const consumeIncomingUrl = (url: string | null, source: string) => {
            ...
            if (entryTarget.type === 'auth') {
                if (url === lastSuccessfulAuthDeepLinkUrlRef.current) {
                    logUiPressProbe('APP_ENTRY_DEEP_LINK_SKIPPED_ALREADY_CONSUMED', { source, url });
                    return;
                }
                if (authDeepLinkInFlightRef.current) {
                    logUiPressProbe('APP_ENTRY_DEEP_LINK_SKIPPED_AUTH_IN_FLIGHT', { source, url });
                    return;
                }

                authDeepLinkInFlightRef.current = true;
                void handleSocialAuthCallback(entryTarget, source)
                    .then((success) => {
                        if (success) {
                            lastSuccessfulAuthDeepLinkUrlRef.current = url;
                            consumedAppEntryDeepLinkUrlRef.current = url;
                            return;
                        }

                        if (consumedAppEntryDeepLinkUrlRef.current === url) {
                            consumedAppEntryDeepLinkUrlRef.current = '';
                        }
                    })
                    .finally(() => {
                        authDeepLinkInFlightRef.current = false;
                    });
                return;
            }
    ...
    const handleAcceptIncomingVoipCall = useCallback(async (sourceVariant: string = 'unknown') => {
        ...
        if (token) {
            try {
                const acceptedFromServer = await acceptIncomingCall(apiBase, token, acceptedPayload.call_id);
                const callerLanguageHint = resolveVoipRemoteLanguageHint(
                    acceptedPayload.display_language,
                    pendingIncomingVoipCallRef.current?.display_language,
                    acceptedFromServer.display_language,
                ...
                const snapshot = await fetchVoipCallResumeSnapshot(apiBase, token, acceptedPayload.call_id);
                logUiPressProbe('VOIP_INCOMING_ACCEPT_API_FAIL', {
                    call_id: acceptedPayload.call_id,
                    error_message: acceptError?.message || 'unknown',
                    snapshot_call_id: snapshot?.call_id ?? null,
    ...
    }, [apiBase, activateAcceptedIncomingVoipCall, dismissPendingIncomingAsMissed, logUiPressProbe, pendingIncomingVoipCall, requestPermissions, setIncomingVoipAcceptInFlight, stopIncomingVoipAlert, token]);
    ...
    const handleRejectIncomingVoipCall = useCallback(async () => {
        ...
        if (token) {
            try {
                await fetch(`${apiBase}/api/v1/voip/calls/${pendingIncomingVoipCall.call_id}/end`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${token}`,
                        'Content-Type': 'application/json',
    ...
    const fetchActiveVoipCallResume = useCallback(async (source: string) => {
        ...
        try {
            ...
            const response = await fetch(`${apiBase}/api/v1/voip/calls/active-current${query}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (!response.ok) {
    ...
    }, [apiBase, applyIncomingVoipPayload, logUiPressProbe, pendingIncomingVoipCall, refreshVoipAudit, setCallMode, token, userInfo, voipCallInitResponse]);
    ...
    const handleFriendAcceptedFromDiscovery = useCallback(async (payload?: AcceptedFriendActionPayload) => {
        ...
        if (payload.action === 'chat') {
            ...
            try {
                const room = await createDirectChatRoom(apiBase, token, payload.friend.friendUserId);
                setSelectedChatRoom(room);
                setShowFriendFolder(false);
    ...
    const handleSearchNearby = useCallback(async () => {
        ...
        clearBookingSelectionNoticeTimer();
        try {
            const places = await callNearbyPlacesApi(apiBase, {
                lat,
                lon,
    ...
    const handleReserveBooking = useCallback(async () => {
        ...
        beginBookingRequestNow({
            setBookingLoading,
            setBookingError,
            setBookingResult,
        });
        try {
            const payload = await callBookingApi(apiBase, token, {
            ...
            applyBookingRequestFailureNow({
                setBookingLoading,
                setBookingError,
                setBookingResult,
            }, e?.message || '예약 요청에 실패했습니다.');
            logUiPressProbe('TRAVEL_BOOKING_SUBMIT_FAIL', {
                selected_place_id: selectedBookingPlace.id,
                message: e?.message || '예약 요청에 실패했습니다.',
            });
        } finally {
            finishBookingRequestNow({
                setBookingLoading,
                setBookingError,
                setBookingResult,
            });
    ...
    const handlePayment = useCallback(async () => {
        ...
        try {
            ...
            const purchase = await callCreatePurchaseApi(apiBase, token, amount);
            setPurchaseResult(purchase);
            const payData = await callInitiatePaymentApi(apiBase, token, purchase.id);
            setPayUrl(payData.payment_url);
    ...
    useEffect(() => {
        ...
        syncRefCurrent(peerLangBootstrappedRef, true);
        void (async () => {
            try {
                ...
                if (raw) {
                    ...
                    if (parsed.manual && isSupportedLangCode(normalized) && normalized !== fromLang) {
                        syncRefCurrent(peerLangManualRef, true);
    ...
    const faceConversationControllerDeps = buildFaceConversationControllerDeps({
        autoVoiceModeEnabled,
        setAutoVoiceModeEnabled,
        fromLang,
        toLang,
        ...
        aiDisplayName,
        getFeatureUiText,
        displayUiText: getDisplayUiText(),
    });

    const { handleToggleFaceConversation } = useFaceConversationController(faceConversationControllerDeps);

    /**
     * 소리새 AI 전용 대화 토글(통역창과 분리). 통역모드의 '상대 언어 필요' 제약을 받지 않고
     * 항상 친구(gpt) 경로로 동작하며, 반이중·자기에코 가드는 공용 캡처 루프 로직을 그대로 재사용한다.
     */
    const { handleToggleSorisaeConversation } = useSorisaeConversationController({
        autoVoiceModeEnabled,
        setAutoVoiceModeEnabled,
        recordingRef,
        stopVoiceInput,
        ...
        aiDisplayName,
        faceModeTexts: {
            webOnlyTitle: aiDisplayName,
            webOnlyBody: getFeatureUiText('sorisae.webChatOnly', { name: aiDisplayName }),
            convStarted: getFeatureUiText('sorisae.convStarted', { name: aiDisplayName }),
            convEnded: getFeatureUiText('sorisae.convEnded', { name: aiDisplayName }),
        },
    });

    /** 소리새 AI 전용 창 닫기 — 진행 중인 대화/재생을 정리하고 닫는다. */
    const { closeSorisaeWindow } = useSorisaeWindowController({
        autoVoiceModeEnabled,
        recordingRef,
        voiceInputTargetRef,
        stopVoiceInput,
    ...
    const { syncSorisaeWindowOpenState } = useSorisaeWindowOpenStateController({
        autoVoiceModeEnabled,
        recordingRef,
        voiceInputTargetRef,
        autoVoiceModeEnabledRef,
    ...
    useEffect(() => {
        void syncSorisaeWindowOpenState(sorisaeWindowOpen);
    }, [sorisaeWindowOpen, syncSorisaeWindowOpenState]);

    /**
     * [Phase6.1] 음성 호출형 — 웨이크워드 감지 시 깨우기 루틴.
     * 소리새 창을 열고 gpt 대화 모드로 전환한다(실제 듣기 재개는 위 창-오픈 이펙트가 처리).
     */
    const { wakeSorisaeVoiceCall: wakeCompanionVoiceCallNow } = useSorisaeWakeController({
        companionVoiceCallRef,
        wakeCompanionVoiceCall,
        faceAiModeRef,
        setFaceAiMode,
        ...
        setGpsStatus,
        aiDisplayName,
        wakeSuccessText: getFeatureUiText('sorisae.wakeSuccess', { name: aiDisplayName }),
    });
    useEffect(() => { syncWakeCompanionVoiceCallNowRef(wakeCompanionVoiceCallNowRef, wakeCompanionVoiceCallNow); }, [wakeCompanionVoiceCallNow]);

    const companionVoiceCallControllerDeps = buildCompanionVoiceCallControllerDeps({
        autoVoiceModeEnabled,
        setAutoVoiceModeEnabled,
        recordingRef,
        stopVoiceInput,
     ...
     */
    const { handleToggleCompanionVoiceCall } = useCompanionVoiceCallController(companionVoiceCallControllerDeps);
    ...
    useSorisaeDormantWatchdog({
        ...
        startVoiceInput,
        closeSorisaeWindow,
        shouldSleep: shouldCompanionVoiceCallSleep,
        sleep: sleepCompanionVoiceCall,
        setGpsStatus,
        dormantStatusText: getFeatureUiText('sorisae.dormant', { name: aiDisplayName }),
    });
    ...
    const handleOpenChatFromContact = useCallback(async (contact: DevicePhoneContact) => {
        ...
        if (!token || !userInfo?.id) {
            void shareChatInvite({ apiBase, contactName: contact.name, inviterName });
            return;
        }
        try {
            ...
            if (action.kind === 'chat' && action.friend.friendUserId != null) {
                const room = await createDirectChatRoom(apiBase, token, action.friend.friendUserId);
                setSelectedChatRoom(room);
                setShowFriendFolder(false);
            ...
            const { shared } = await shareChatInvite({ apiBase, contactName: contact.name, inviterName });
            setInterCallStatus(
                shared
    ...
    const handleChatContact = useCallback(async (contact: DeviceContact, friend: Friend | null) => {
        ...
        if (friend && friend.friendUserId != null && token) {
            try {
                const room = await createDirectChatRoom(apiBase, token, friend.friendUserId);
                setSelectedChatRoom(room);
                setShowFriendFolder(false);
                setActiveRailSection('chat');
                setInterCallStatus(getFeatureUiText('pstn.chatOpened', { name: contact.name }));
    ...
    useEffect(() => { syncRefCurrent(faceScreenOpenRef, faceScreenOpen); }, [faceScreenOpen]);
    ...
    const isSorisaeFabVisible = Platform.OS !== 'web'
        && !!userInfo
        && !showLogin
        && !voipCallInitResponse
        ...
        && !sorisaeWindowOpen
        && globalSettings.sorisaeFab;
    const lastSorisaeFabVisibleRef = useRef<boolean | null>(null);
    ...
    useEffect(() => {
        // 소리새 FAB 노출 판단을 로그로 남겨, 콜백 후 FAB 복귀 여부를 증거 번들에서 바로 확인한다.
        if (lastSorisaeFabVisibleRef.current === isSorisaeFabVisible) {
            return;
        }
        lastSorisaeFabVisibleRef.current = isSorisaeFabVisible;
        logUiPressProbe('SORISAE_FAB_VISIBLE_EVAL', {
            visible: isSorisaeFabVisible,
            platform: Platform.OS,
            sorisae_fab_setting: globalSettings.sorisaeFab,
            pending_incoming_voip: hasPendingIncomingVoip,
            voip_call_active: Boolean(voipCallInitResponse),
            sorisae_window_open: sorisaeWindowOpen,
        });
    }, [
    ...
    useFaceConversationAutoVoiceGuards({
        autoVoiceModeEnabled,
        toLang,
        fromLang,
        faceScreenOpen,
        ...
        setAutoVoiceModeEnabled,
        setGpsStatus,
        getPeerRequiredText: () => (getDisplayUiText().faceConversationPeerRequired ?? '상대 언어를 GPS 또는 수동 선택으로 지정해 주세요.'),
        ...
        <ImageBackground source={SKY_BG} resizeMode="cover" style={styles.skyBg}>
            <SafeAreaView style={styles.root} edges={['top', 'left', 'right']}>
                ...
                <FaceInterpretationScreen
                    ...
                    homeToFlag={homeToFlag}
                    peerLangManual={peerLangManual}
                    resultText={resultText}
                    inputText={inputText}
                    autoVoiceModeEnabled={autoVoiceModeEnabled}
                    getFeatureUiText={getFeatureUiText}
                    peerLanguageHintText={getDisplayUiText().peerLanguageHint}
                    onOpenPeerLangPicker={() => openPeerLangPicker()}
                    onToggleFaceConversation={() => {
                        if (Platform.OS !== 'web') {
                            void handleToggleFaceConversation();
                        }
                    }}
                    onRequestClose={() => {
                        setLangPickerFor(null);
                        setFaceScreenOpen(false);
                        if (autoVoiceModeEnabled && Platform.OS !== 'web') {
                            void handleToggleFaceConversation();
                        }
                    }}
                    onOpenChat={() => {
                            ...
                            <View style={styles.loginCard} accessibilityLabel="worldlinco-login-modal" testID="worldlinco-login-modal">
                                ...
                                {authModalMode === 'login' ? (
                                    <View style={styles.socialLoginRow}>
                                        {SOCIAL_LOGIN_PROVIDER_CONFIGS.map((config) => (
                                            <Pressable
                                                key={config.provider}
                                                style={[styles.socialLoginButton, { borderColor: config.accentColor, backgroundColor: `${config.accentColor}18` }]}
                                                onPress={() => { void handleSocialLoginStart(config.provider); }}
                                                accessibilityRole="button"
                                                accessibilityLabel={`worldlinco-social-login-${config.provider}`}
                                                testID={`worldlinco-social-login-${config.provider}`}
                                            >
                                                <Text style={styles.socialLoginIcon}>{config.icon}</Text>
                                                <Text style={[styles.socialLoginLabel, { color: config.accentColor }]}>{config.label}</Text>
                                            </Pressable>
                                        ))}
                                    </View>
                                ) : null}
