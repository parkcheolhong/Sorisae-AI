...
#
# 툴체인 자동 감지:
#   - MSVC(cl.exe) 가 있으면 기본 생성기(Visual Studio / NMake)로 빌드.
#   - 없으면 MinGW-w64(g++ / mingw32-make)를 찾아 "MinGW Makefiles" 로 빌드.
#       * 자립형 .pyd(정적 libstdc++/libgcc)로 링크되어 런타임 DLL 의존이 없다(CMakeLists 의 MINGW 분기).
#       * 개발 PC 에 없으면:  winget install --id BrechtSanders.WinLibs.POSIX.UCRT
