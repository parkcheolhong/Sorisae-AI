...
def test_empty_book_fallbacks():
    ...
    assert fv.volume_spike == pytest.approx(1.0)  # 첫 틱 → 1.0
    assert fv.vwap == pytest.approx(100.0)        # vol=0 → price 폴백
    assert fv.obi_norm == pytest.approx(0.0)      # 표본<2 → 0
    assert fv.micro_momentum == pytest.approx(0.0)
...
def test_one_sided_book_obi():
    ...
    assert fv.mid_price == pytest.approx(100.0)
...
def test_zero_and_negative_volume_spike():
    ...
    assert fv2.volume_spike == pytest.approx(1.0)
...
def test_detection_flat_on_empty_book():
    ...
    fv = fe.update(MarketTick(ts_ns=1, symbol="E", bids=(), asks=(), last_price=100.0, last_qty=0.0))
    sig = det.evaluate(fv)
    assert sig.side.value == "flat" and sig.confidence == pytest.approx(0.0)
