...
- `WS_2차_CONNECT=OK`, `WS_2차_PONG={"event":"pong", ...}`
...
- 증적: 1차/2차 리포트 아티팩트 + Step Summary 표
...
- `1차`: `state_status=ok`, `overall=ok`, `recommended=extras`, `decide_iot_domain=extras-iot`, `decide_unknown_fallback=true`
- `2차`: `state_status=ok`, `overall=ok`, `recommended=extras`, `decide_game_domain=extras-game`, `decide_policy_status=denied`, `decide_policy_denied=true`
- `PRODUCTION`: `login=ok`, `state_status=ok`, `overall=ok`, `recommended=extras`
- `ADMIN_UI_1차`: `/admin` 로그인 후 우측 레일 카드 `CT OK · REC extras`, `IOT extras-iot · GAME extras-game`, `UNKNOWN fallback ON` 렌더링 확인
- `ADMIN_UI_2차`: `/admin` 새로고침 후 동일 카드 값 재확인, React runtime error #310 미재현
