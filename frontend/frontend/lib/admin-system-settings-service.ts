...
export interface AdminSocialLoginRuntimeProviderDetail {
    provider: string;
    ready: boolean;
    missing_keys: string[];
}

export interface AdminSocialLoginRuntimeStatus {
    enabled_providers: string[];
    ready_providers: string[];
    provider_details: AdminSocialLoginRuntimeProviderDetail[];
    callback_base_url: string;
    all_enabled_ready: boolean;
}

export interface AdminSystemSettingsRuntimeApply {
    applied: boolean;
...
export function buildSystemSettingsOpenState(settings: AdminSystemSettingsResponse, previous: Record<string, boolean>) {
    ...
    settings.sections.forEach((section) => {
        nextOpen[section.id] = previous[section.id] ?? section.id === 'worldlinco_public_portal';
    });
    return nextOpen;
}
