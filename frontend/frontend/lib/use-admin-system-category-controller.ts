...
export function useAdminSystemCategoryController(options: UseAdminSystemCategoryControllerOptions) {
    ...
    const loadSystemSettings = useCallback(async (options?: { forceDraftSync?: boolean }) => {
        const token = getAdminToken();
        if (!token) {
            return;
        }
        ...
        try {
            ...
            const hasUnsavedChanges = !!systemSettings && (() => {
                ...
                for (const key of allKeys) {
                    if (String(persistedValues[key] ?? '') !== String(systemSettingsDraft?.[key] ?? '')) {
                        return true;
                    }
                }
                return false;
            ...
            if (!hasUnsavedChanges || options?.forceDraftSync) {
    ...
    }, [apiBaseUrl, handleAdminUnauthorized, normalizeSystemSettingsMessage, pushLiveLog, systemSettings, systemSettingsDraft]);
