...
const orchestratorWebServerUrl = `http://localhost:${orchestratorE2ePort}`;
const webServerUrl = useOrchestratorDedicatedServer
    ? orchestratorWebServerUrl
    : (process.env.PLAYWRIGHT_ADMIN_BASE_URL ?? `http://localhost:${adminPort}`);
...
module.exports = defineConfig({
    ...
    webServer: shouldStartWebServer
        ? {
            command: `npm run dev -- --hostname localhost --port ${adminPort}`,
