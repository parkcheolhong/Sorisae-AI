...
export interface AdminSystemSettingsPanelProps {
    ...
    systemSettingsHasUnsavedChanges?: boolean;
...
export default function AdminSystemSettingsPanel({
    ...
    return (
        <>
            ...
            {systemSettingsHasUnsavedChanges ? (
                <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-medium text-amber-900">
                    저장되지 않은 변경사항이 있습니다. `설정 새로고침` 전에 `.env 저장`을 먼저 눌러주세요.
                </div>
            ) : null}
            ...
            {systemSettingsLoading && !systemSettings ? (
                ...
                <div className="space-y-3">
                    {systemSettings.social_login_runtime ? (
                        <div className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-4 text-sky-950">
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                <div>
                                    <h3 className="text-sm font-semibold">소셜 로그인 패널</h3>
                                    <p className="mt-1 text-xs text-sky-900/80">
                                        구글/카카오/네이버 OAuth 공급자 활성화와 인증키 준비 상태를 즉시 확인합니다.
                                    </p>
                                </div>
                                <span className={`rounded-full border px-2 py-1 text-[11px] font-semibold ${systemSettings.social_login_runtime.all_enabled_ready ? 'border-emerald-300 bg-white text-emerald-700' : 'border-amber-300 bg-white text-amber-700'}`}>
                                    {systemSettings.social_login_runtime.all_enabled_ready ? 'all providers ready' : '추가 인증키 필요'}
                                </span>
                            </div>
                            <div className="mt-3 grid gap-2 md:grid-cols-3">
                                {systemSettings.social_login_runtime.provider_details.map((detail) => (
                                    <div key={detail.provider} className={`rounded-lg border px-3 py-2 text-xs ${detail.ready ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
                                        <p className="font-semibold uppercase">{detail.provider}</p>
                                        <p className="mt-1">{detail.ready ? 'ready' : `missing: ${detail.missing_keys.join(', ')}`}</p>
                                    </div>
                                ))}
                            </div>
                            <p className="mt-3 text-[11px] text-sky-900/80">
                                활성 공급자: {(systemSettings.social_login_runtime.enabled_providers || []).join(', ') || '-'}
                                {' · '}callback base: {systemSettings.social_login_runtime.callback_base_url || '(미설정)'}
                            </p>
                        </div>
                    ) : null}
                            ...
                            <div key={section.id} className="overflow-hidden rounded-xl border border-gray-200">
                                ...
                                {isOpen && (
                                    <div className="border-t border-gray-200 px-4 py-4">
                                        {section.id === 'social_login_provider' ? (
                                            <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs text-emerald-900">
                                                <p className="font-semibold">소셜 로그인 저장</p>
                                                <p className="mt-1">공급자/클라이언트 키 입력 후 아래 버튼을 눌러 즉시 .env에 반영하세요.</p>
                                                <div className="mt-3 flex flex-wrap items-center gap-2">
                                                    <button
                                                        ...
                                                        disabled={systemSettingsLoading || systemSettingsSaving || systemSettingsFillingMissing || systemAutomaticApplying || !systemSettings}
                                                    >
                                                        {systemSettingsSaving ? '저장 중...' : '소셜 인증키 저장'}
                                                    </button>
                                                    <span className="text-[11px] text-emerald-800">저장 후 런타임 상태 카드에서 준비 상태를 확인하세요.</span>
                                                </div>
                                            </div>
                                        ) : null}
                                        ...
                                        {section.id === 'worldlinco_public_portal' ? (
                                            <div className="mb-4 rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-xs text-sky-900">
                                                ...
                                                <p className="mt-1 font-semibold text-rose-700">주의: 항공 API 인증키를 공공데이터 포털 서비스키 칸에 입력하면 안 됩니다.</p>
                                                <p className="mt-1">1. `공공데이터 포털 조회 사용 여부 (0/1)` 은 `1` 로 켭니다.</p>
                                                <p className="mt-1">2. 실시간 항공은 `실시간 항공 포털 URL 템플릿` 과 `실시간 항공 포털 서비스키` 칸을 따로 채웁니다.</p>
                                                <p className="mt-1">3. 관광/문화/식당/숙소는 TourAPI 템플릿을 넣고, 약국·병원·교통은 각 포털 API를 별도 주입합니다.</p>
                                                <p className="mt-1">4. 공통으로 쓰는 키가 있으면 `공공데이터 공통 서비스키` 에 넣고, 전용 키가 있으면 각 분기 칸에 따로 넣습니다.</p>
                                                <p className="mt-1 break-all">지원 플레이스홀더: {`{service_key}`}, {`{latitude}`}, {`{longitude}`}, {`{radius_m}`}, {`{category}`}, {`{query}`}, {`{country_code}`}, {`{medical_q0}`}, {`{medical_q1}`}, {`{medical_qd}`}, {`{medical_qt}`}, {`{medical_qn}`}, {`{medical_qz}`}</p>
                                                <p className="mt-1 break-all">실시간 항공 예시 템플릿: https://apis.data.go.kr/B551178/flight-search?serviceKey={`{service_key}`}</p>
                                                ...
                                                <p className="mt-1 break-all">병·의원 템플릿 예시: https://apis.data.go.kr/B552657/HsptlAsembySearchService/getHsptlMdcncListInfoInqire?serviceKey={`{service_key}`}&pageNo=1&numOfRows=10&Q0={`{medical_q0}`}&Q1={`{medical_q1}`}&QD={`{medical_qd}`}&QT={`{medical_qt}`}&QN={`{medical_qn}`}&QZ={`{medical_qz}`}&ORD=NAME</p>
                                                <p className="mt-1">병·의원 자동 필터: 질의에 지역명이 있으면 Q0/Q1, 진료과목명이 있으면 QD, 요일이 있으면 QT, 병원/의원 표현이 있으면 QZ로 자동 매핑합니다.</p>
                                                <p className="mt-1 break-all">교통/환승 템플릿 예시: https://apis.data.go.kr/1613000/PublicTransportationMode/getPublicTransportationMode?serviceKey={`{service_key}`}&pageNo=1&numOfRows=10&dataType=JSON</p>
                                                <p className="mt-1">분기 권장: 실시간 항공은 `실시간 항공 포털 URL 템플릿`, 관광/문화/식당/숙소는 `관광/문화/식당/숙소 포털 URL 템플릿`, 약국/병원은 `약국/병원 포털 URL 템플릿`, 교통은 `교통/환승 포털 URL 템플릿` 을 사용하세요.</p>
                                                <div className="mt-3 flex flex-wrap gap-2">
                                                    <button
                                                        ...
                                                        disabled={systemSettingsLoading || systemSettingsSaving || systemSettingsFillingMissing || systemAutomaticApplying || !systemSettings}
                                                    >
                                                        {systemSettingsSaving ? '저장 중...' : '네이버 API 등록 저장'}
                                                    </button>
                                                    <span className="inline-flex items-center text-[11px] text-sky-800">
                                                        입력 후 이 버튼으로 바로 .env 저장
                                                    </span>
                                                </div>
                                            </div>
                                        ) : null}
                                                    ...
                                                    {section.id === 'worldlinco_public_portal' ? (
                                                        <div className="mt-2 flex items-center gap-2">
                                                            <button
                                                                type="button"
                                                                onClick={onSaveSystemSettings}
                                                                className="rounded-md border border-emerald-300 bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-800 hover:bg-emerald-100 disabled:opacity-60"
                                                                disabled={systemSettingsLoading || systemSettingsSaving || systemSettingsFillingMissing || systemAutomaticApplying || !systemSettings}
                                                            >
                                                                {systemSettingsSaving ? '저장 중...' : '이 항목 저장'}
                                                            </button>
                                                            <span className="text-[11px] text-gray-500">현재 편집값 포함 .env 저장</span>
                                                        </div>
                                                    ) : null}
                                                </label>
                                            ))}
