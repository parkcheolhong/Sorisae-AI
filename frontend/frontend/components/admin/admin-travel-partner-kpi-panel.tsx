...
export default function AdminTravelPartnerKpiPanel() {
    ...
    async function fetchKpi(options?: { silent?: boolean; syncThresholds?: boolean }) {
        const silent = options?.silent === true;
        const syncThresholds = options?.syncThresholds === true;
        if (!silent) {
            setLoading(true);
            setMessage(null);
        }
        ...
        try {
            ...
            if (syncThresholds && data.ops?.settings?.thresholds) {
    ...
    async function handleLoad() {
        await fetchKpi({ silent: false, syncThresholds: true });
    }
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                // ignore initial settings failure; KPI fetch will provide visible error
            }
            await fetchKpi({ silent: true, syncThresholds: false });
        })();

        const timer = setInterval(() => {
            void fetchKpi({ silent: true, syncThresholds: false });
        }, 30000);
    ...
    return (
        <div className="workspace-section-stack" data-testid="admin-travel-partner-kpi-panel">
            <div className="workspace-sidebar-card">
                ...
                <p style={{ marginTop: 8, fontSize: 12, color: 'rgba(125,211,252,0.82)' }}>
                    연결 상태: {payload ? '연결됨' : '대기 중'}
                    {connectedAt ? ` · last_sync ${connectedAt}` : ''}
                </p>
            ...
            <div className="workspace-sidebar-card" data-testid="travel-kpi-sla-card">
                ...
                {!payload || payload.sla.length === 0 ? (
                    <p style={{ marginTop: 8, fontSize: 13, color: 'rgba(255,255,255,0.68)' }}>
                        {payload ? '데이터 연결됨 · 집계된 예약 이벤트가 없습니다.' : '데이터 연결 대기 중입니다.'}
                    </p>
                ) : (
            ...
            <div className="workspace-sidebar-card" data-testid="travel-kpi-alert-summary">
                ...
                {!payload?.ops?.alert_summary ? (
                    <p style={{ marginTop: 8, fontSize: 13, color: 'rgba(255,255,255,0.68)' }}>
                        {payload ? '데이터 연결됨 · 현재 표시할 운영 알림이 없습니다.' : 'KPI 데이터 연결 대기 중입니다.'}
                    </p>
                ) : (
                    <>
