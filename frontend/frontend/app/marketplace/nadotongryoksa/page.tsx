...
async function callLogoutApi(apiBase: string, token: string): Promise<void> {
    ...
    if (!res.ok) {
        ...
        throw new Error(payload?.detail || `로그아웃 실패 (HTTP ${res.status})`);
    }
}
...
const UI_KO_TEXT = {
    ...
    login: '로그인',
    close: '닫기',
    signupGuidePrefix: '계정이 없으면',
    signup: '회원가입',
    appTitle: 'WorldLinco',
    appSubtitle: '신세계소리새 AI · 통번역 · 지도 · 예약 · 결제',
    offlineBadge: '🔴 오프라인',
    myInfo: '내 정보',
    loadingDots: '⏳ 불러오는 중...',
    hideHistory: '📋 내역 닫기',
    bookingHistory: '📋 구매/예약 내역',
    noPurchases: '구매 내역이 없습니다.',
    logout: '로그아웃',
    sourceLanguage: '원본 언어',
    gpsDetectTitle: '현재 위치로 번역 언어 자동 감지',
    gpsDetectLoading: '⏳ 감지 중...',
    gpsDetect: '🌐 GPS 언어 감지',
    inputPlaceholder: '번역할 텍스트를 입력하세요 (Ctrl+Enter로 번역)',
    voiceInput: '음성 입력',
    readAloud: '읽기',
    swapLanguage: '⇄ 언어 스왑',
    translating: '번역 중...',
    ...
    translatingWithAi: '신세계소리새 AI 번역 중...',
    copy: '복사',
    resultPlaceholder: '번역 결과가 여기에 표시됩니다',
    offlineNotice: '📡 오프라인 모드 — 백엔드 연결 후 신세계소리새 전체 통역 엔진 사용 가능',
    endCall: '📵 통역 통화 종료',
    startCall: '📞 통역 통화 시작',
    inCall: '통화 중',
    preparing: '준비 중...',
    nearbyTitle: '📍 주변 검색',
    nearbyDesc: '호텔 · 공항 · 식당 · 관광명소를 현재 좌표 기준으로 찾습니다.',
    categoryAll: '전체',
    categoryHotel: '호텔',
    categoryAirport: '공항',
    categoryRestaurant: '식당',
    categoryAttraction: '관광명소',
    currentLocation: '현재 위치',
    latitude: '위도',
    longitude: '경도',
    searchingNearby: '주변 검색 중...',
    findNearby: '주변 장소 찾기',
    map: '🗺️ 지도',
    naverMap: 'Naver 지도',
    ...
    closeMap: '✕ 닫기',
    mapTitlePrefix: '지도',
    openHotelLocation: '호텔 위치 열기',
    loadingReviews: '리뷰 조회 중...',
    googleReviews: 'Google 리뷰',
    hotelBooking: '호텔 예약',
    review: '리뷰',
    noReviewSummary: '요약이 없습니다.',
    hotelPanelTitle: '🏨 호텔 예약 패널',
    hotelPanelDesc: '주변검색 결과에서 호텔을 선택하면 예약 요청을 보낼 수 있습니다.',
    viewOnMap: '🗺️ 지도에서 보기',
    selectBookableHotel: '주변검색 결과에서 예약 가능한 호텔을 선택하세요.',
    bookingName: '예약자명',
    checkin: '체크인',
    checkout: '체크아웃',
    guests: '인원',
    roomCount: '객실 수',
    bookingNote: '추가 요청사항 (예: 금연실, 늦은 체크인)',
    bookingLoading: '예약 요청 중...',
    sendBookingRequest: '예약 요청 보내기',
    bookingConfirmation: '예약 확인번호',
    translatedMessage: '현지어 안내',
    supportPhone: '고객센터',
    openGoogleMap: 'Google Maps 열기',
    paymentPanelTitle: '💳 결제 패널',
    paymentPanelDesc: '예약 확정 후 결제를 진행하면 바우처가 활성화됩니다.',
    selectHotelForPayment: '예약 대상 호텔을 먼저 선택해 주세요.',
    paymentAmount: '결제 예정 금액',
    purchaseIdStatus: '구매 ID',
    status: '상태',
    nights: '박',
    currencyWon: '원',
    openPaymentPage: '🔗 결제 페이지 열기',
    loadingPaymentUrl: '결제 URL을 불러오는 중...',
    paying: '결제 처리 중...',
    proceedPayment: '💳 결제 진행하기',
    loginForPayment: '🔐 로그인 후 결제',
    paymentLoginRequiredPrefix: '결제를 진행하려면',
    paymentLoginRequiredSuffix: '이 필요합니다.',
    mobileInstallTitle: '📱 모바일 앱 설치',
    mobileInstallDesc: 'Android 기기에 설치해 오프라인에서도 사용하세요.',
    apkDownload: '📥 APK 다운로드',
    backToMarketplace: '마켓플레이스로 돌아가기',
    apkGuideLine1: 'APK 패키지에는 Expo React Native 소스 + EAS 빌드 가이드가 포함됩니다.',
...
const t = useCallback((key: keyof typeof UI_KO_TEXT) => uiText[key] || UI_KO_TEXT[key], [uiText]);
    ...
    const handleLogout = async () => {
        try {
            ...
            if (currentToken) {
                await callLogoutApi(API_BASE, currentToken);
            }
        } catch {
    ...
    const resolveBrowserCurrentLocation = useCallback(() => new Promise<{ latitude: number; longitude: number }>((resolve, reject) => {
        if (typeof window === 'undefined' || !navigator.geolocation) {
    ...
    const handleSearchNearby = useCallback(async () => {
        ...
        try {
            let resolvedLat = lat.trim();
            let resolvedLon = lon.trim();
            try {
                const currentLocation = await resolveBrowserCurrentLocation();
                resolvedLat = currentLocation.latitude.toFixed(6);
                resolvedLon = currentLocation.longitude.toFixed(6);
                setLat(resolvedLat);
                setLon(resolvedLon);
            } catch {
                if (!resolvedLat || !resolvedLon) {
                    throw new Error('위도와 경도를 입력해 주세요.');
                }
            }
            const places = await callNearbyPlacesApi({ lat: resolvedLat, lon: resolvedLon, category: nearbyCategory, radiusM, targetLang: to });
    ...
    }, [lat, lon, nearbyCategory, radiusM, resolveBrowserCurrentLocation, to]);
    ...
    const handleApkDownload = async () => {
        try {
            const preferredPath = apkManifest?.downloadPath || `${WORLDLINGO_MARKETPLACE_API_PREFIX}/latest.apk`;
            const downloadUrl = preferredPath.startsWith('http') ? preferredPath : `${API_BASE}${preferredPath}`;
            window.location.href = downloadUrl;
        } catch {
            alert('네트워크 오류로 APK 다운로드를 시작하지 못했습니다.');
        }
    };
            ...
            <div style={{ maxWidth: 960, margin: '0 auto', padding: '20px 16px 40px' }}>
                ...
                <div style={{ background: '#151b23', border: '1px solid #21262d', borderRadius: 12, padding: 20, marginTop: 24 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                        <button onClick={handleApkDownload} style={{ display: 'inline-block', background: '#2a7cff', color: '#fff', padding: '10px 20px', borderRadius: 10, fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer' }}>📥 APK 다운로드</button>
