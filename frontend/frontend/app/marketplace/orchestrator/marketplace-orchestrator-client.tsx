...
export default function MarketplaceOrchestratorClient({
    ...
    const resolveBrowserCurrentLocation = React.useCallback(() => new Promise<{ latitude: number; longitude: number }>((resolve, reject) => {
    ...
    const sendStageChat = React.useCallback(async (contentOverride?: string, speakerOverride?: string) => {
        ...
        try {
            const browserLocation = await resolveBrowserCurrentLocation().catch(() => null);
