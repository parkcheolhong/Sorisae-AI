...
function AdminRecoveryPageContent() {
  ...
  const [sessionClearBusy, setSessionClearBusy] = useState(false);
  ...
  const clearActiveSession = async () => {
    if (!resetToken) {
      setError("세션 해제 전에 본인 확인을 완료해주세요.");
      return;
    }

    setSessionClearBusy(true);
    setError("");
    setMessage("");
    try {
      const response = await fetch(`${apiBaseUrl}/api/auth/recovery/clear-active-session`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          scope: recoveryScope,
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload.detail || `세션 해제 실패 (HTTP ${response.status})`);
      }

      setMessage("잔존 활성 세션을 해제했습니다. 이제 현재 기기에서 다시 로그인해주세요.");
      setResetToken("");
      setVerificationCode("");
      setNewPassword("");
      setStep("request");
    } catch (e: any) {
      setError(e.message || "세션 해제에 실패했습니다.");
    } finally {
      setSessionClearBusy(false);
    }
  };
      ...
      <div className="w-full max-w-[520px] rounded-2xl bg-white px-10 py-12 shadow-[0_20px_60px_rgba(0,0,0,0.2)]">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-[#666]">
            로그인 전에 이메일 또는 휴대폰 인증으로 계정을 확인할 수 있습니다.
          </p>
        </div>
        ...
        {step === "complete" && (
          <div className="space-y-4">
            <div className="rounded-xl border border-[#ffe4b8] bg-[#fffaf0] px-4 py-4">
              <div className="mb-2 text-sm font-semibold text-[#7c5400]">잔존 세션 강제 해제</div>
              <p className="mb-3 text-xs text-[#8a6d00]">
                다른 기기/브라우저에 남아 있는 로그인 세션 때문에 409가 발생하면, 먼저 이 버튼으로 세션을 해제하세요.
              </p>
              <button
                type="button"
                onClick={() => void clearActiveSession()}
                disabled={!resetToken || sessionClearBusy || loading || passkeyBusy}
                data-testid="admin-recovery-clear-active-session"
                className={`w-full rounded-lg border px-4 py-[14px] text-base font-semibold ${resetToken && !sessionClearBusy ? "border-[#f0ad4e] bg-white text-[#8a6d00] hover:bg-[#fff6e8]" : "cursor-not-allowed border-[#ecd8b5] bg-[#f7f2e8] text-[#b39a6a]"}`}
              >
                {sessionClearBusy ? "⏳ 세션 해제 중..." : "🧹 잔존 세션 강제 해제"}
              </button>
            </div>
        ...
        <div className="mt-6 text-center text-sm">
          <Link href={returnTo} className="font-medium text-[#5b67d8] underline underline-offset-2">
            로그인으로 돌아가기
          </Link>
        </div>
      </div>
