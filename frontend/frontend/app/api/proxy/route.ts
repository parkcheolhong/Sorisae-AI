...
const ADMIN_REGRESSION_MOCK_BACKEND = envEnabled(process.env.ADMIN_REGRESSION_MOCK_BACKEND)
  && envEnabled(process.env.CI)
...
function hostFromOrigin(origin: string): { protocol: string; host: string } | null {
  try {
...
function resolvePublicOrigin(req: NextRequest): string {
  const explicitOrigin = firstHeaderValue(req.headers.get('origin'));
  const fromOrigin = hostFromOrigin(explicitOrigin);
  ...
  const selectedHost = hostCandidates.find((host) => !loopbackHost(host)) || hostCandidates[0] || nextHost;
  const forwardedProto = firstHeaderValue(req.headers.get('x-forwarded-proto'));
  const selectedProto = (fromOrigin?.protocol || forwardedProto || (loopbackHost(selectedHost) ? 'http' : 'https')).replace(':', '');

  return `${selectedProto}://${selectedHost}`;
}
...
export async function PATCH(req: NextRequest) {
  ...
  const explicitOrigin = firstHeaderValue(req.headers.get('origin'));
  const parsedExplicitOrigin = hostFromOrigin(explicitOrigin);
  const origin = parsedExplicitOrigin && !loopbackHost(parsedExplicitOrigin.host)
    ? explicitOrigin
    : publicOrigin;
  const referer = req.headers.get('referer') || `${origin}${req.nextUrl.pathname}`;

  const parsedOrigin = hostFromOrigin(origin);
  const forwardedHost = parsedOrigin?.host || req.nextUrl.host;
  const forwardedProto = parsedOrigin?.protocol || req.nextUrl.protocol.replace(':', '');
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      return jsonNoStore(parsedBody ?? { detail: bodyText || '로그아웃에 실패했습니다.' }, response.status);
    }

    return jsonNoStore(parsedBody ?? { ok: true }, response.status);
  } catch (e: any) {
    const error = e?.error ?? e;
    return jsonNoStore(
      {
        error: isAbortLike(error)
          ? `백엔드 로그아웃 응답이 ${ADMIN_PROXY_TIMEOUT_MS / 1000}초 안에 오지 않았습니다.`
          : `백엔드 연결 실패: ${error.message}`,
        backend: e?.target || resolveBackendBaseUrl(),
      },
      isAbortLike(error) ? 504 : 502,
